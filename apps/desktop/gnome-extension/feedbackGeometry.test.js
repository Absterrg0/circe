...
it("activates only a window of the process authenticated by D-Bus", () => {
  const target = window(42, "Circe");
  expect(findCaptureDestination([window(99, "Circe"), target], 42, "Circe")).toBe(target);
  expect(findCaptureDestination([window(99, "Circe")], 42, "Circe")).toBeUndefined();
