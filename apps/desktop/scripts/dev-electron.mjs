...
const remoteDebuggingPort = process.env.CIRCE_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line circe/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.CIRCE_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.CIRCE_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--circe-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--circe-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
