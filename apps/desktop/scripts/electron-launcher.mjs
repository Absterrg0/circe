// This file mostly exists because we want dev mode to say "Circe (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
const APP_DISPLAY_NAME = isDevelopment ? "Circe (Dev)" : "Circe";
const APP_BUNDLE_ID = isDevelopment
  ? `com.abstergo.circe.dev.${devBundleIdSuffix || "local"}`
  : "com.abstergo.circe";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["circe-dev"] : ["circe"];
...
// oxlint-disable-next-line circe/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentLauncherScript({
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --circe-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
