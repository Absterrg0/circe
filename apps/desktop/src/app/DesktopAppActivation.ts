...
import { HostProcessUserId } from "@circe/shared/hostProcess";
  ...
  const server = NodeNet.createServer((socket) => {
    ...
    socket.on("data", (chunk) => {
      ...
      void input.handle(parsed).then(finish, () => {
        finish(
          invalidResponse(parsed.requestId, "Circe could not process the desktop app request."),
        );
      });
    });
...
>()("@circe/desktop/app/DesktopAppActivation") {}
