...
describe("DesktopBackendConfiguration", () => {
  ...
  it.effect("prefers the external packaged resource monitor over the copy inside the asar", () =>
    Effect.gen(function* () {
      ...
      const embeddedMonitorPath = `${resourcesPath}/app.asar/apps/desktop/prod-resources/resource-monitor/circe-resource-monitor`;
      const monitorPath = path.join(resourcesPath, "resource-monitor/circe-resource-monitor");
  ...
  it.effect("prefers the release resource monitor when both development builds exist", () =>
    Effect.gen(function* () {
      ...
      const releaseMonitorPath = path.join(
        baseDir,
        "native/resource-monitor/target/release/circe-resource-monitor",
      );
      const debugMonitorPath = path.join(
        baseDir,
        "native/resource-monitor/target/debug/circe-resource-monitor",
      );
  ...
  it("resolvePrimaryLabel is runSync-safe against the real WSL availability probe", async () => {
    ...
    // oxlint-disable-next-line circe/no-manual-effect-runtime-in-tests -- This test intentionally replicates the sync IPC handler's runSync path to catch a regression to async-only resolution; it.effect would mask it.
    ...
    try {
      ...
      // oxlint-disable-next-line circe/no-manual-effect-runtime-in-tests -- Same reason: this is the synchronous resolution the IPC handler performs.
