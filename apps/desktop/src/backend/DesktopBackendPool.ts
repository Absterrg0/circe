...
//     /.well-known/circe/environment (Windows vs Linux platform).
...
>()("@circe/desktop/backend/DesktopBackendPool") {}
      ...
      function* (failure: DesktopBackendManager.PreflightFailure) {
        ...
        if (!fatal) {
          ...
          yield* electronDialog.showErrorBox(
            ...
            `${reason}\n\nThe desktop app will use the Windows backend for this launch and retry WSL the next time the app starts.`,
          );
        ...
        yield* electronDialog.showErrorBox(
          ...
          `${reason}\n\nFalling back to the Windows backend so the desktop app can open. Re-enable the WSL backend from Settings > Connections once the WSL distro is fixed.`,
        );
