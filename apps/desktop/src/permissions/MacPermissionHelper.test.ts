...
vi.mock("electron", async () => {
  ...
  return {
    app: {
      getPath: () => "/Applications/Circe (Nightly).app/Contents/MacOS/Circe",
    },
...
describe("macAppBundlePath", () => {
  it("resolves bundles with spaces and refuses non-bundle executables", () => {
    expect(macAppBundlePath("/Applications/Circe.app/Contents/MacOS/Circe")).toBe(
      "/Applications/Circe.app",
    );
    expect(macAppBundlePath("/usr/local/bin/electron")).toBeUndefined();
    expect(macAppBundlePath("/Applications/Circe.app/other/MacOS/Circe")).toBeUndefined();
  });
});
it("drags the running app bundle only for the helper's own renderer", async () => {
  ...
  expect(mocks.startDrag).toHaveBeenCalledWith({
    file: "/Applications/Circe (Nightly).app",
    icon: mocks.createFromPath.mock.results[0]!.value.resize(),
  });
  send("finder");
  expect(mocks.showItemInFolder).toHaveBeenCalledWith("/Applications/Circe (Nightly).app");
});
...
it("does not show a helper with a missing packaged icon", async () => {
  ...
  await expect(open()).rejects.toThrow("packaged Circe icon is missing");
  expect(windows).toHaveLength(0);
});
