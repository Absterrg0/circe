...
<header>↑ Drag Circe into the list above</header>
<button id="app" draggable="true" aria-label="Drag Circe to System Settings, or click to reveal in Finder"><img src="${escapeHtml(icon)}" alt="" draggable="false">Circe</button>
</main></body></html>`;
}
...
export class MacPermissionHelper {
  ...
  async show(
    ...
    if (!appIcon) throw new Error("The packaged Circe icon is missing.");
