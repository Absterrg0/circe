...
const OVERLAY_ATTRIBUTE = "data-circe-annotation-ui";
...
function startAnnotation(): void {
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-circe-annotation-tool", tool);
  };
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-circe-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;
