...
const MAC_ACCESSIBILITY_PERMISSION_MESSAGE =
  "Allow Accessibility in System Settings, then restart Circe.";
const MAC_BOTH_PERMISSIONS_MESSAGE =
  "Allow Accessibility and Screen Recording in System Settings, then restart Circe.";
...
export class DesktopSnapShot extends Context.Service<
  DesktopSnapShot,
  {
    ...
    /** Capture the foreground window in place, including Circe itself. */
...
>()("@circe/desktop/snapShot/DesktopSnapShot") {}
...
export const make = Effect.gen(function* () {
    ...
    return yield* Effect.gen(function* () {
      ...
      if (snapshot.linuxActivationFailure) {
        yield* Effect.logWarning(
          "The compositor could not activate Circe after the snapshot",
  ...
  const checkShortcut = Effect.fn("desktop.snapShot.checkShortcut")(function* (
    ...
    if (mode === "portal" && niriSocketPath()) {
      return {
        available: false,
        message: "Configure the capture shortcut in your Niri config, not in Circe.",
      };
    }
    ...
    if (mode === "portal" && niriSocketPath()) {
      const registered = yield* Effect.tryPromise(async () => {
        ...
        return startNiriCaptureShortcut(linuxAppId, onCurrentShortcut, () => {
          void runPromise(
            setShortcutFailure("The Niri capture endpoint disconnected. Restart Circe."),
          ).catch(() => undefined);
        });
      }).pipe(
        Effect.tap((stop) =>
      ...
      yield* Ref.set(stateRef, {
        ...
        shortcutMessage: registered
          ...
          : "Could not start the Niri capture endpoint. Another Circe instance may be using it.",
        message: null,
      });
      return;
    }
