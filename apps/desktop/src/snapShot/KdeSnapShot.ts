...
import type { DesktopCaptureHelperState } from "@circe/contracts";
...
export const KDE_CAPTURE_EXECUTABLE = "circe-kde-snap-shot";
const DESKTOP_FILE = "com.abstergo.circe.KdeCapture.desktop";
const MARKER = "X-Circe-Capture-Helper=true";
...
export function kdeCapturePaths(paths: KdeCapturePaths) {
  return {
    executable: NodePath.join(paths.dataHome, "circe", "kde-capture", KDE_CAPTURE_EXECUTABLE),
...
export class KdeCaptureSetup {
  ...
  async state(): Promise<DesktopCaptureHelperState> {
    try {
      ...
      if (!bundle)
        return {
          status: "error",
          message: "The capture helper is missing from this build. Update or reinstall Circe.",
        };
  ...
  async perform(action: "install-kde-helper" | "remove-kde-helper") {
    ...
    if (action === "remove-kde-helper") {
      ...
      if (!bundle)
        throw new Error(
          "The capture helper is missing from this build. Update or reinstall Circe.",
        );
