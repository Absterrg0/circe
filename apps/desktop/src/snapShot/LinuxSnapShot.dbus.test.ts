...
it.runIf(hasDbus)("captures through real D-Bus marshalling on a private bus", async () => {
  ...
  try {
    ...
    await server.requestName("org.gnome.Shell.Extensions.CirceSnapShot", NameFlag.DO_NOT_QUEUE);
    ...
    expect(await portal.backend("com.abstergo.circe")).toBe("screenshot-portal");
    expect(await portal.capturePortal()).toEqual({ png });
    expect(target).toBe(8);
    portalVersion = 2;
    const extension = connect();
    expect(await extension.backend("com.abstergo.circe")).toBe("gnome-extension");
    expect(await extension.captureExtension("com.abstergo.circe")).toMatchObject({
      png,
      window: { processId: 42 },
    });
    const owner = await server.call(
      new Message({
        ...
        member: "GetNameOwner",
        signature: "s",
        body: ["com.abstergo.circe.SnapShot"],
      }),
    );
    ...
    expect(await updated.backend("com.abstergo.circe")).toBe("gnome-extension");
    const snapshot = await updated.captureExtension("com.abstergo.circe", {
      flash: true,
      animate: true,
    });
    expect(snapshot.feedback?.animationStarted).toBe(true);
    await snapshot.feedback!.activate("Circe");
    ...
    expect(activateTitle).toBe("Circe");
    expect(animateFrame).toEqual([0.1, 0.8, 0.2, 0.1]);
    vi.stubEnv("XDG_CURRENT_DESKTOP", "KDE");
    const kde = connect();
    expect(await kde.backend("com.abstergo.circe")).toBe("kde");
    ...
    const command = [
      ...
      "--session",
      "--dest",
      "com.abstergo.circe.NiriTest.SnapShot",
      "--object-path",
      "/com/abstergo/circe/SnapShot",
      "--method",
      "com.abstergo.circe.SnapShot.Capture",
    ];
    ...
    const invalid = server.call(
      new Message({
        destination: "com.abstergo.circe.NiriTest.SnapShot",
        path: "/com/abstergo/circe/SnapShot",
        interface: "com.abstergo.circe.SnapShot",
        member: "Capture",
        signature: "s",
        body: ["not allowed"],
      }),
