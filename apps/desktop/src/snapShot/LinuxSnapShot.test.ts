...
const appId = "com.abstergo.circe";
const metadata = {
  title: "Editor",
  appName: "Text Editor",
...
it("retains the authenticated connection until the compositor flight has landed", async () => {
  ...
  expect(
    bus.calls
      ...
      .map((call) => call.body),
  ).toEqual([[false, true], ["Circe"], [0.1, 0.8, 0.2, 0.1]]);
  expect(bus.disconnect).toHaveBeenCalledOnce();
  feedback.close();
  expect(bus.disconnect).toHaveBeenCalledOnce();
});
...
it("can activate without animation, and expires a renderer-abandoned capture", async () => {
  ...
  await expect(snapshot!.feedback!.activate("Circe")).rejects.toThrow("No window");
