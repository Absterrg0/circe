...
import * as SshAuth from "@circe/ssh/auth";
import { resolveSshTarget } from "@circe/ssh/command";
import { discoverSshHosts } from "@circe/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@circe/ssh/errors";
import * as SshTunnel from "@circe/ssh/tunnel";
...
>()("@circe/desktop/ssh/DesktopSshEnvironment") {}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "The desktop app window is not available for SSH authentication.";
      break;
