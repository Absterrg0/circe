...
const SERVER_ENTRY_SOURCE = '#!/bin/sh\necho "circe wsl runtime test server 0.0.0"\n';
...
describe("WSL runtime cache", () => {
  ...
  it("installs through a temporary directory and only reuses valid completed caches", () => {
    const script = buildWslRuntimeInstallScript(
      "/mnt/c/Program Files/Circe/wsl-runtime.tar.gz",
      "1.2.3-x64",
      "b".repeat(64),
    );

    expect(script).toContain('runtime_parent="$HOME/.circe/wsl-runtime"');
    ...
    expect(script).toContain(
      "tar -xzf '/mnt/c/Program Files/Circe/wsl-runtime.tar.gz' -C \"$runtime_tmp\" --strip-components=1",
    );
  ...
  describe("formatNodePtyProbeFailureReason", () => {
    it("identifies a packaged build that omitted the Linux node-pty prebuild", () => {
      ...
      expect(reason).not.toContain("Circe");
    });
  ...
  it("verifies the archive digest before extracting, and only on a cache miss", () => {
    ...
    expect(script).toContain(
      "archive_sha=$(sha256sum '/mnt/c/Program Files/Circe/wsl-runtime.tar.gz' | cut -d ' ' -f 1)",
    );
  ...
  it("re-hashes the executable against the digest the install recorded", () => {
    ...
    expect(script).toContain(
      `printf '%s\\n' "$installed_entry_digest" > "$runtime_tmp/.circe-wsl-runtime-ready"`,
    );
    ...
    const markerWritten = script.indexOf('> "$runtime_tmp/.circe-wsl-runtime-ready"');
  ...
  it("parses only absolute Linux runtime paths", () => {
    expect(parseWslRuntimeRoot("runtimeRoot:/home/josh/.circe/wsl-runtime/1.2.3-x64\n")).toBe(
      "/home/josh/.circe/wsl-runtime/1.2.3-x64",
    );
  ...
  it("prunes completed runtimes except the current and newest previous cache", () => {
    ...
    expect(script).toContain('[ -f "$candidate/.circe-wsl-runtime-ready" ] || continue');
  ...
  it("invalidates a cache by dropping its ready marker, not the tree", () => {
    ...
    expect(script).toContain('rm -f "$HOME/.circe/wsl-runtime/1.2.3_x64/.circe-wsl-runtime-ready"');
...
describe.skipIf(posixShellRunner === null)("WSL runtime install script (executed)", () => {
  ...
  const createFixture = () => {
    ...
    return {
      ...
      archiveSha,
      runtimeId,
      runtimeParent: `${work}/home/.circe/wsl-runtime`,
      runtimeRoot: `${work}/home/.circe/wsl-runtime/${runtimeId}`,
      serverEntry: `${work}/home/.circe/wsl-runtime/${runtimeId}/t3`,
      installScript,
  ...
  it("extracts once when two installs race for the same cache", () => {
    ...
    const raced = runShell(
      [
        ...
        'PATH="$work/bin:$PATH"',
        "export PATH",
        `cat > "$work/install.sh" <<'CIRCE_INSTALL_SCRIPT'`,
        fixture.installScript(),
        "CIRCE_INSTALL_SCRIPT",
  ...
  it("leases a warm cache across the prepare-to-spawn handoff", () => {
    ...
    const selected = runShell(
      [
        ...
        'printf ready > "$runtime_parent/sha256-current/.circe-wsl-runtime-ready"',
        'printf ready > "$runtime_parent/sha256-previous/.circe-wsl-runtime-ready"',
        `touch -d "10 minutes ago" ${sh(fixture.runtimeRoot)}`,
        'touch -d "1 minute ago" "$runtime_parent/sha256-previous"',
        `cat > ${sh(`${fixture.work}/select.sh`)} <<'CIRCE_SELECT_SCRIPT'`,
        fixture.installScript(),
        "CIRCE_SELECT_SCRIPT",
  ...
  it("prunes a selected cache after its prepare-to-spawn grace period expires", () => {
    ...
    const result = runShell(
      [
        ...
        'printf ready > "$runtime_parent/sha256-previous/.circe-wsl-runtime-ready"',
        `touch -d "10 minutes ago" ${sh(fixture.runtimeRoot)}`,
        `touch -d "10 minutes ago" ${sh(`${fixture.runtimeRoot}/.circe-wsl-runtime-selected`)}`,
        'touch -d "1 minute ago" "$runtime_parent/sha256-previous"',
        `HOME=${sh(`${fixture.work}/home`)}`,
        "export HOME",
        buildWslRuntimePruneScript("sha256-current"),
  ...
  it("prunes old and markerless caches without touching retained, active, locked, or unrelated roots", () => {
    const result = runShell(
      [
        ...
        "work=$(mktemp -d)",
        'home="$work/home"',
        'runtime_parent="$home/.circe/wsl-runtime"',
        'mkdir -p "$runtime_parent"',
        'make_ready() { mkdir -p "$runtime_parent/$1"; printf ready > "$runtime_parent/$1/.circe-wsl-runtime-ready"; }',
