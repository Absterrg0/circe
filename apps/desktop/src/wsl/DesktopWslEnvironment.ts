...
import { buildRemoteNodeEnvScript } from "@circe/ssh/tunnel";
import { satisfiesSemverRange } from "@circe/shared/semver";
...
>()("@circe/desktop/wsl/DesktopWslEnvironment") {}
...
// that install wrote.
const WSL_RUNTIME_READY_MARKER = ".circe-wsl-runtime-ready";
const WSL_RUNTIME_SELECTED_MARKER = ".circe-wsl-runtime-selected";
...
export const buildWslRuntimeInvalidateScript = (runtimeId: string): string => {
  ...
  return [
    "set -eu",
    `rm -f "$HOME/.circe/wsl-runtime/${safeRuntimeId}/${WSL_RUNTIME_READY_MARKER}"`,
  ].join("\n");
};
...
/** Exported for the unit test; production callers use the probe path. */
export const formatNodePtyProbeFailureReason = (exitCode: number): string | null =>
  exitCode === NODE_PTY_BINARY_MISSING_EXIT_CODE
    ? "WSL support is missing from this desktop app build: the packaged Linux node-pty binary was not included. Rebuild the Windows artifact with `--wsl-prebuild <path-to-linux-pty.node>` or install a build that includes WSL support."
    : null;
