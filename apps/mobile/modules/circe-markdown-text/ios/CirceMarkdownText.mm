...
#import <react/renderer/components/CirceMarkdownTextSpec/EventEmitters.h>
#import <react/renderer/components/CirceMarkdownTextSpec/Props.h>
#import <react/renderer/components/CirceMarkdownTextSpec/RCTComponentViewHelpers.h>
...
static void CirceMarkdownTextApplyParagraphStyles(
    NSMutableAttributedString *attributedString,
    const std::vector<CirceMarkdownTextParagraphStyleRange> &styleRanges)
{
...
static void CirceMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
    const std::vector<CirceMarkdownTextAttachmentRange> &attachmentRanges,
    NSDictionary<NSString *, UIImage *> *images)
{
  for (const auto &attachmentRange : attachmentRanges) {
    ...
    const CGFloat attachmentSize = CirceMarkdownTextAttachmentSize(attachmentRange);
    ...
    [attributedString replaceCharactersInRange:range
                          withAttributedString:CirceMarkdownTextAttachmentString(attachment, runAttributes)];
  }
}
...
@interface CirceMarkdownText () <RCTCirceMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface CirceMarkdownText () <T3MarkdownOutsideTapTarget>
@end
...
{
  return concreteComponentDescriptorProvider<CirceMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const CirceMarkdownTextProps>();
    _props = defaultProps;
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<CirceMarkdownTextProps const>(_props);
  ...
  CirceMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  CirceMarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  for (UIView *child in self.subviews) {
    if (![child isKindOfClass:[CirceMarkdownTextRun class]]) {
      continue;
    }

    CirceMarkdownTextRun *textChild = (CirceMarkdownTextRun *)child;
  ...
  for (UIView *child in self.subviews) {
    if (![child isKindOfClass:CirceMarkdownTextRun.class]) continue;
    CirceMarkdownTextRun *run = (CirceMarkdownTextRun *)child;
  ...
  for (const auto &attachmentRange : _state->getData().attachmentRanges) {
    ...
    CirceMarkdownTextRun *run = [self childForCharacterRange:range];
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::CirceMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::CirceMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<CirceMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
...
{
  const auto &oldViewProps = *std::static_pointer_cast<CirceMarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<CirceMarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == CirceMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == CirceMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == CirceMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == CirceMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const CirceMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (nullable CirceMarkdownTextRun *)childForCharacterRange:(NSRange)characterRange
{
...
- (CirceMarkdownTextRun*)getTouchChild:(CGPoint)location
{
...
{
  CirceMarkdownTextRun *child = [self childForCharacterRange:textItem.range];
  if (![child hasContextMenu] && !child.contextChipInteractive) {
    return defaultAction;
  }

  __weak CirceMarkdownTextRun *weakChild = child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::CirceMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::CirceMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> CirceMarkdownTextCls(void)
{
  return CirceMarkdownText.class;
}
