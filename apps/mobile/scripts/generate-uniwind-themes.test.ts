...
describe("generate mobile Uniwind themes", () => {
  it("keeps the committed outputs current", () => {
    ...
    expect(
      staleOutputs,
      "Run `vp run --filter @circe/mobile generate` and commit the generated outputs.",
    ).toEqual([]);
  });
  ...
  it("generates the default runtime bridge from the authored CSS", () => {
    ...
    expect(variables.light["--color-screen"]).toBe("#fcf9f4");
    expect(variables.dark["--color-screen"]).toBe("#0c0d0e");
