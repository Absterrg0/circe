...
describe("resolveNativeReviewDiffView", () => {
  ...
  it("returns the payload bridge when the native review diff view is installed", async () => {
    ...
    expect(expoMocks.requireNativeView).toHaveBeenCalledWith("CirceReviewDiffSurface");
  });
  ...
  it("returns null when the view manager cannot be required", async () => {
    ...
    expect(consoleError).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        nativeModuleName: "CirceReviewDiffSurface",
        cause,
      }),
    );
  ...
  it("recognizes registration races for the installed native view name", async () => {
    ...
    expect(
      isPendingNativeViewRegistration(
        new Error("Unable to find the 'CirceReviewDiffSurface' view for this native tag"),
      ),
    ).toBe(true);
    expect(
      isPendingNativeViewRegistration(
        new Error("Unable to find the 'CirceReviewDiffView' view for this native tag"),
      ),
    ).toBe(false);
    expect(
      isPendingNativeViewRegistration(
        new Error(
          "Unable to find the class expo.modules.t3reviewdiff.CirceReviewDiffView view with tag 1150",
        ),
      ),
    ).toBe(true);
  });
