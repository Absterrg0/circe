...
import { BUILT_IN_THEMES, getThemeColorsForAppearance } from "@circe/shared/themePalettes";
...
describe("getMobileTerminalTheme", () => {
  it("preserves the default light terminal palette", () => {
    expect(getMobileTerminalTheme("circe", "light")).toMatchObject({
      background: "#f2f2f7",
      foreground: "#6C6C71",
  ...
  it("preserves the default dark terminal palette", () => {
    expect(getMobileTerminalTheme("circe", "dark")).toMatchObject({
      background: "#0a0a0a",
      foreground: "#adadb1",
      cursorForeground: "#009fff",
  ...
  it("applies the selected palette without replacing ANSI status colors", () => {
    const standard = getMobileTerminalTheme("circe", "dark");
...
describe("buildGhosttyThemeConfig", () => {
  it("serializes theme colors into a ghostty config file", () => {
    const config = buildGhosttyThemeConfig(getMobileTerminalTheme("circe", "dark"));
