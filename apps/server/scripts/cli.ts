...
const buildExeCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      yield* Effect.log(
        "[cli] Built dist-exe/circe (expects client/, resource-monitor/, and the runtime-external node_modules beside it; scripts/build-cli-archive.ts assembles that tree)",
      );
    }),
).pipe(
 ...
 * every `@absterrg0/circe-<platform>.tgz` first, `@absterrg0/circe.tgz` (the launcher) last, so
...
const publishCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      const scopeDir = path.join(packagesDir, "@absterrg0");
      const launcherTarball = path.join(scopeDir, "circe.tgz");
      const platformTarballs = (yield* fs
        .readDirectory(scopeDir)
        .pipe(Effect.orElseSucceed((): ReadonlyArray<string> => [])))
        .filter((entry) => entry.startsWith("circe-") && entry.endsWith(".tgz"))
        .sort()
        .map((entry) => path.join(scopeDir, entry));
      if (platformTarballs.length === 0) {
        return yield* new ServerCliBuildAssetMissingError({
          assetPath: path.join(scopeDir, "circe-<platform>.tgz"),
        });
      }
  ...
  Command.withDescription(
    "Publish the @absterrg0/circe-<platform> tarballs and then the @absterrg0/circe launcher to npm.",
  ),
);
...
const cli = Command.make("cli").pipe(
  Command.withDescription("Circe server build & publish CLI."),
