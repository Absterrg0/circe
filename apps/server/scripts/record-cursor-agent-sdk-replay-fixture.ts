...
async function prepareWorkspace(scenario: RecordingName): Promise<{
  ...
  const configuredCwd = process.env.CIRCE_CURSOR_REPLAY_CWD?.trim();
  if (configuredCwd) {
    return {
      cwd: configuredCwd,
      owned: false,
...
const scenario = (readArgValue("--scenario") ?? process.env.CIRCE_CURSOR_REPLAY_SCENARIO) as
  | RecordingName
  | undefined;
...
try {
  ...
  const transcript = await recordCursorAgentSdkReplayTranscript({
    ...
    modelSelection: {
      ...CURSOR_MODEL_SELECTION,
      model: process.env.CIRCE_CURSOR_REPLAY_MODEL ?? CURSOR_MODEL_SELECTION.model,
    },
    cwd: workspace.cwd,
