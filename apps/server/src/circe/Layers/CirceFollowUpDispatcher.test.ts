...
import { OrchestratorDispatchError, OrchestratorV2 } from "../../orchestration-v2/Orchestrator.ts";
...
function harness(
  ...
  const orchestratorLayer = Layer.mock(OrchestratorV2)({
    ...
    getThreadProjection: () =>
      Effect.succeed(hooks?.projection?.() ?? projectionWithRun("running")),
  ...
  () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        expect(
          harness.commands.filter((command) => command.type === "message.dispatch"),
        ).toHaveLength(1);
