...
/**
 * `circe acp-mcp-bridge` — internal stdio MCP server that ACP agents spawn.
 *
 ...
 * reaches the circe-code toolkit through ACP's required stdio MCP transport.
...
export const acpMcpBridgeCommand = Command.make("acp-mcp-bridge").pipe(
  Command.withDescription("Bridge Circe's MCP endpoint to stdio for ACP agents."),
  Command.unlisted,
...
export const acpMcpCallCommand = Command.make("acp-mcp-call", {
  ...
  Command.withDescription("Call one Circe MCP tool from an ACP agent terminal."),
  Command.unlisted,
