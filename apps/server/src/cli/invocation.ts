...
import { HostProcessArguments } from "@circe/shared/hostProcess";
 ...
 * Detection is best-effort; callers must fail closed to a plain `circe` command.
 */
...
/**
 * The `circe` package spec to suggest. The literal spec the user typed (e.g.
 * `@absterrg0/circe@nightly`) is resolved away before our process starts, so re-derive it
...
function suggestedPackageSpec(version: string): string {
  ...
  return channel === undefined ? "@absterrg0/circe" : `@absterrg0/circe@${channel}`;
}

/**
 * Render a `circe <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx @absterrg0/circe connect` suggests
 * `npx @absterrg0/circe serve`, a global install suggests `circe serve`, and a nightly build
