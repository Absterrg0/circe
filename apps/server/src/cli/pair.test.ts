...
describe("circe pair", () => {
  it.effect("mints a token and prints a QR pairing URL for a live server", () =>
    withDescriptorServer((origin) =>
      Effect.gen(function* () {
        const baseDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "circe-pair-test-"));
        ...
        assert.equal(credentials[0]?.label, "circe pair");
      }),
    ).pipe(
  ...
  it.effect("pairs through the recorded dev web URL for dev servers", () =>
    withDescriptorServer((origin) =>
      Effect.gen(function* () {
        const baseDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "circe-pair-dev-test-"));
        const port = Number(new URL(origin).port);
  ...
  it.effect("directs to circe serve or circe connect when no server is running", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "circe-pair-none-test-"));
      ...
      assert.include(rendered, "No running Circe server found.");
      assert.include(rendered, "npx @absterrg0/circe serve");
      assert.include(rendered, "npx @absterrg0/circe connect");
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("ignores runtime state whose recorded pid is no longer alive", () =>
    withDescriptorServer((origin) =>
      Effect.gen(function* () {
        const baseDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "circe-pair-pid-test-"));
        const statePath = NodePath.join(baseDir, "userdata", "server-runtime.json");
  ...
  it.effect("ignores stale runtime state pointing at a dead server", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "circe-pair-stale-test-"));
      const statePath = NodePath.join(baseDir, "userdata", "server-runtime.json");
