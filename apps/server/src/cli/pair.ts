/**
 * `circe pair` - mint a pairing token for an already-running server and print it
...
import { resolveWorktreeCirceHome } from "@t3tools/shared/devHome";
import {
...
export class NoRunningServerError extends Schema.TaggedError<NoRunningServerError>()(
  ...
  override get message(): string {
    return [
      "No running Circe server found.",
      ...this.checkedStatePaths.map((statePath) => `  checked ${statePath}`),
      "Start one with `npx @absterrg0/circe serve`, or connect this machine with Circe Mesh: `npx @absterrg0/circe connect`.",
    ].join("\n");
  }
}
...
export class ServesOtherEnvironmentError extends Schema.TaggedError<ServesOtherEnvironmentError>()(
  ...
  override get message(): string {
    return `Tailscale Serve on HTTPS port ${String(this.servePort)} already fronts a different Circe server. Pass --tailscale-serve-port to publish this one on another port.`;
  }
}
...
export class ServePortOccupiedError extends Schema.TaggedError<ServePortOccupiedError>()(
  ...
  override get message(): string {
    return `HTTPS port ${String(this.servePort)} on the tailnet already serves something that is not a Circe server. Pass --tailscale-serve-port to publish this one on another port.`;
  }
}
...
const probeEnvironmentDescriptor = (
  ...
  Effect.gen(function* () {
    ...
    // it as unreachable lets `circe pair --tailscale` repair its own mapping
...
const mintPairingLink = Effect.fn("pair.mintPairingLink")(function* (input: {
  ...
  return yield* Effect.gen(function* () {
    ...
    return yield* environmentAuth.createPairingLink({
      ...
      label: Option.getOrElse(input.label, () => "circe pair"),
...
export const pairCommand = Command.make("pair", {
  ...
  Command.withDescription(
    "Mint a pairing token for a running Circe server and print it as a QR code.",
  ),
