...
const status = {
  ...
  installed: true,
  current: true,
  unitPath: "/home/me/.config/systemd/user/circe.service",
...
it("reports the installed service version and host paths", () => {
  assert.equal(
    ...
    [
      "Circe service",
      "  Status: installed · @absterrg0/circe@0.0.29",
      "  Unit: /home/me/.config/systemd/user/circe.service",
...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    "Next: Run `circe service install` to repair it.",
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain("Run `circe service install` to repair it.");
...
it("reports a newer installed service and tells the CLI to catch up to it", () => {
  ...
  assert.include(
    output,
    "@absterrg0/circe@0.0.32-nightly.1 (newer than this @absterrg0/circe@0.0.31 CLI)",
  );
  assert.include(output, "Run `circe update 0.0.32-nightly.1` to match it");
