...
export function formatServiceStatus(
  ...
  if (
    ...
    return [
      ...
      `  Logs: ${status.logPath}`,
      ...problems,
      `  Next: Run \`circe update ${installedVersion}\` to match it, or pass \`--allow-downgrade\` to \`circe service install\` explicitly.`,
    ].join("\n");
  }
  return [
    "Circe service",
    ...
    `  Logs: ${status.logPath}`,
    ...problems,
    ...(status.current ? [] : ["  Next: Run `circe service install` to repair it."]),
  ].join("\n");
}
...
// Kept one release for muscle memory and old docs. It did what `circe service
// install` does; the way to move to a newer release is `circe update`.
const serviceUpdateCommand = Command.make("update", serviceReconcileFlags).pipe(
  Command.withDescription("Deprecated. Run `circe update` to move to a newer release."),
  Command.unlisted,
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        yield* Console.log(
          "`circe service update` is deprecated: run `circe update` to move to a newer release, or `circe service install` to repair the service. Repairing now.",
        );
        const result = yield* reconcileService({ allowDowngrade: flags.allowDowngrade });
        if (!result.changed) {
          yield* Console.log(
...
const serviceRestartCommand = Command.make("restart", projectLocationFlags).pipe(
  Command.withDescription(
    "Restart the background service. Picks up a version installed by `circe update` that was not restarted at the time.",
  ),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          restarted
            ? `Restarted the Circe service${status.installedVersion === undefined ? "" : ` on @absterrg0/circe@${status.installedVersion}`}.`
            : "Circe service is not installed.",
        );
      }),
    ),
  ),
