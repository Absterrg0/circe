/**
 * `circe triage` - hand a misbehaving install to the user's own coding agent.
 *
...
import { isCommandAvailable, resolveSpawnCommand } from "@circe/shared/shell";
...
export const triageCommand = Command.make("triage", {
  ...
  Command.withDescription(
    "Investigate an Circe problem on this machine with claude or codex, and help file a good issue.",
  ),
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      // --base-dir wins; CIRCE_HOME is its documented env equivalent (same
      // precedence as `circe pair`).
