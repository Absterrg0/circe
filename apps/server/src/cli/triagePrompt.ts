...
export const TRIAGE_PLAYBOOK = `# Circe triage playbook

You are a support engineer for Circe (https://github.com/Absterrg0/Circe), working
...
Fetch https://raw.githubusercontent.com/Absterrg0/Circe/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/Absterrg0/Circe <source-cache-dir>/<hash>
...
- How is Circe running on this machine: \`npx @absterrg0/circe serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: the web app served by their node,
  the desktop app against a local server, the desktop app against a remote server,
  or the mobile app?
...
Search existing issues in Absterrg0/Circe (use \`gh\`, or the public GitHub search
...
Do not patch the Circe source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/Absterrg0/Circe/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Circe triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Circe user is \
having a problem with their install and started this session with \`circe triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# Circe triage context

Generated by \`circe triage\` at ${input.generatedAt}.
...
- Repo: https://github.com/Absterrg0/Circe
