...
/**
 * What `circe uninstall` would remove for one T3 home. Computed before anything
...
export interface UninstallPlan {
  ...
  /** The `circe` launcher (symlink or `.cmd` shim) that points into this home's runtime tree. */
...
export const uninstallCommand = Command.make("uninstall", {
  ...
  Command.withDescription(
    "Remove circe from this machine: the background service, the launcher, and every downloaded version. Your projects and threads are kept.",
  ),
...
const runUninstall = Effect.fn("cli.uninstall.run")(function* (input: {
  ...
  if (!plan.service && plan.launcher === undefined && plan.runtimeDir === undefined) {
    yield* Console.log(`Nothing to remove: circe is not installed for ${input.baseDir}.`);
    if (!(yield* HostProcessIsExecutable)) {
      yield* Console.log(
        "  This circe runs from a Node script, so it was installed by npm or built from source. Remove it the same way (`npm uninstall -g circe`, or delete the checkout).",
      );
    }
    return;
  }
  ...
  if (!input.assumeYes) {
    ...
    const confirmed = yield* Prompt.run(
      Prompt.confirm({ message: "Remove circe from this machine?", initial: false }),
  ...
  if (plan.runtimeDir !== undefined) {
    ...
    if (platform === "win32") {
      ...
      yield* Console.log(`${runtimeDir} will be removed once circe exits.`);
    } else {
      yield* fs
  ...
  yield* Console.log("");
  yield* Console.log("circe is uninstalled. Thanks for trying Circe.");
});
