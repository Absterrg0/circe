...
it.layer(NodeServices.layer)("circe update launcher", (it) => {
  it.effect("repoints a symlink that lives in a runtime versions tree", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "circe-update-" });
      const oldExe = path.join(root, "runtime/versions/1.0.0/circe");
      const newExe = path.join(root, "runtime/versions/2.0.0/circe");
      const launcher = path.join(root, "bin/circe");
  ...
  it.effect("leaves a plain copy or a foreign symlink alone", () =>
    Effect.gen(function* () {
      ...
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "circe-update-" });
      const newExe = path.join(root, "runtime/versions/2.0.0/circe");
      const copy = path.join(root, "copy/circe");
      const foreign = path.join(root, "foreign/circe");
      const elsewhere = path.join(root, "elsewhere/circe");
      // Another install's versions tree: same shape, different home.
      const otherHome = path.join(root, "other/runtime/versions/1.0.0/circe");
      const otherLauncher = path.join(root, "other/bin/circe");
  ...
  it.effect("finds the launcher a bare command name resolved to on PATH", () =>
    Effect.gen(function* () {
      ...
      const bare = yield* resolveLauncherPath.pipe(
        Effect.provideService(HostProcessInvokedAs, "circe"),
      ...
      const relative = yield* resolveLauncherPath.pipe(
        Effect.provideService(HostProcessInvokedAs, "./bin/circe"),
