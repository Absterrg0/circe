  ...
  for (let page = 1; page <= RELEASE_INDEX_MAX_PAGES; page += 1) {
    const body = yield* httpClient
      ...
      .pipe(
        ...
        Effect.mapError(() => new CliUpdateError({ reason: "Could not list circe releases." })),
        Effect.timeoutOrElse({
          duration: RELEASE_INDEX_TIMEOUT,
          orElse: () =>
            Effect.fail(new CliUpdateError({ reason: "Timed out listing circe releases." })),
        }),
      );
    const releases = yield* decodeReleaseIndex(body).pipe(
      Effect.mapError(
        () => new CliUpdateError({ reason: "The circe release index had an unexpected shape." }),
      ),
    );
...
/**
 * The launcher the install scripts leave behind: a symlink at `<bin>/circe` on
 * POSIX, a `circe.cmd` shim on Windows. `circe update` repoints it so the next `circe`
...
export const repointLauncher = Effect.fn("cli.update.repoint_launcher")(function* (input: {
  ...
  if (platform === "win32") {
    ...
    // itself as argv0; the shim is the `circe.cmd` next to it only when launched
    // from an install script's bin directory. Find it by searching the
    // directories that would resolve `circe` on this shell's PATH.
 ...
 * spelling in argv0: a launcher symlink or `./circe` resolves against the
 * working directory, while a bare `circe` was found on PATH and has to be
 ...
 * only ever sees its own path. Walk PATH for a `circe.cmd` whose target is the
...
export const findWindowsShim = Effect.fn("cli.update.find_windows_shim")(function* (
  ...
  for (const directory of candidates) {
    const shimPath = path.join(directory, "circe.cmd");
...
const updateFlags = {
  ...
  channel: Flag.choice("channel", CLI_RELEASE_CHANNELS).pipe(
    Flag.withDescription(
      "Release channel to follow. Defaults to the channel this circe was published on.",
    ),
    Flag.optional,
  ),
...
export const updateCommand = Command.make("update", {
  ...
  Command.withDescription(
    "Download a newer circe and switch this machine to it, including the background service when one is installed.",
  ),
...
/**
 * A `circe serve` or `circe` someone started by hand, as opposed to the one the
...
const runUpdate = Effect.fn("cli.update.run")(function* (input: {
  ...
  if (input.requestedVersion !== undefined && !isExactServiceVersion(input.requestedVersion)) {
    return yield* new CliUpdateError({
      reason: `'${input.requestedVersion}' is not an exact circe version.`,
    });
  }
  ...
  if (targetChannel === "preview" && currentChannel !== "preview") {
    yield* Console.log(
      [
        `circe@${targetVersion} is a preview build.`,
        ...
        `  will have to switch back to ${currentChannel} yourself with \`circe update --channel ${currentChannel} --allow-downgrade\`.`,
      ].join("\n"),
    );
  ...
  if (executableCurrent && serviceCurrent) {
    yield* Console.log(
      serviceVersion !== undefined
        ? `circe and its background service are already on ${targetVersion} (${targetChannel}).`
        : `circe is already on ${targetVersion} (${targetChannel}).`,
    );
    return;
  }
  if (!input.allowDowngrade && compareExactServiceVersions(targetVersion, newestInstalled) < 0) {
    return yield* new CliUpdateError({
      reason: `circe@${targetVersion} is older than the installed ${newestInstalled}. Pass --allow-downgrade to install it anyway.`,
    });
  }
  ...
  if (serviceInstalled && !serviceCurrent) {
    ...
    if (input.assumeYes) {
      ...
      yield* Console.log(
        "  Not a terminal, so the service keeps running its current version. Rerun with --yes to restart it now, or run `circe service restart` later.",
      );
    }
  }
  ...
  const runtime = yield* ensurePinnedRuntimeInstalled({
    ...
    validate: (paths) =>
      runner
        ...
        .pipe(
          Effect.mapError(
            (cause) =>
              new PinnedRuntimeInstallError({ step: "verifying the downloaded circe", cause }),
          ),
            ...
            result.code === 0 && /\bv(\S+)\s*$/.exec(result.stdout)?.[1] === targetVersion
              ...
              : Effect.fail(
                  new PinnedRuntimeInstallError({
                    step: "verifying the downloaded circe",
    ...
    Effect.catchIf(
      (error): error is PinnedRuntimeInstallError =>
        ...
        error.step.startsWith("downloading the circe release checksums") &&
        String(error.cause).includes("404"),
      () =>
        Effect.fail(
          new CliUpdateError({
            reason: `No release archive was published for circe@${targetVersion}.`,
          }),
        ),
    ),
  );
  ...
  // is rewritten either way so a later `circe service restart` lands on the new
  ...
  if (serviceInstalled && !serviceCurrent) {
    ...
    yield* BootService.BootService.pipe(
      ...
      Effect.mapError(
        (error) =>
          new CliUpdateError({
            reason: `circe@${targetVersion} is installed but the background service could not be ${restartService ? "updated" : "pointed at it"}: ${error.message}`,
          }),
      ),
    );
  ...
  progress.success(`Installed Circe ${targetVersion}`);
  if (Option.isSome(repointed)) {
    yield* Console.log("  Run circe to get started.\n");
  } else {
  ...
  if (serviceUpdated) {
    ...
    yield* Console.log(
      `  Background service still running ${serviceVersion ?? "an unknown version"}. Run \`circe service restart\` when you are ready to switch it to ${targetVersion}.`,
    );
