...
import { CLI_RELEASE_BASE_URL_ENV } from "@circe/shared/cliRelease";
...
/**
 * Reads `CIRCE_HOME` back out of a rendered unit or plist. Only values this
...
export function bootServiceBaseDirOf(contents: string): string | undefined {
  const systemd = /^Environment=CIRCE_HOME=(.*)$/m.exec(contents)?.[1];
  ...
  const plist = /<key>CIRCE_HOME<\/key>\s*<string>([^<]*)<\/string>/.exec(contents)?.[1];
  if (plist !== undefined) {
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    ...
    "Type=simple",
    "WorkingDirectory=%h",
    `Environment=CIRCE_HOME=${quoteSystemdValue(plan.baseDir)}`,
...
export class BootServiceInstallError extends Schema.TaggedError<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the Circe background service.";
  }
}
...
export function formatBootServiceProblem(problem: BootServiceProblem): string {
  switch (problem) {
    ...
    case "service-disabled":
      return "The service is not enabled to start automatically. Run `circe service install` to repair it.";
    case "service-stopped":
      return "The service is not running. Check the service log and `systemctl --user status circe.service`, then run `circe service install`.";
    case "restart-pending":
      return "A newer version is installed but the service is still running the previous one. Run `circe service restart` to switch.";
  }
}
...
export class BootServiceDowngradeRefusedError extends Schema.TaggedError<BootServiceDowngradeRefusedError>()(
  ...
  override get message(): string {
    return `Refusing to replace @absterrg0/circe@${this.installedVersion} with older @absterrg0/circe@${this.targetVersion}. Run the command again with --allow-downgrade to continue.`;
  }
}
...
>()("@absterrg0/circe/cloud/bootService") {}
