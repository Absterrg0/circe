...
import { isSecureRelayUrl } from "@circe/shared/relayUrl";
...
const reconcileDesiredCloudLinkWith = Effect.fn("environment.cloud.reconcileDesiredLinkWith")(
  ...
  Effect.catchIf(
    ServerSecretStore.isSecretStoreError,
    failEnvironmentCloudInternalError("Could not persist desired Circe Mesh link state."),
  ),
  Effect.catchTags({
...
// explicit launcher stop (`circe service uninstall`, `systemctl stop`,
