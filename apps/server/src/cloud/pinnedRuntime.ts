...
/**
 * A pinned runtime is an exact circe release archive unpacked into
 ...
 * Node or npm on the machine; the only npm involvement in Circe is the
 * `@absterrg0/circe` package for people who prefer `npx @absterrg0/circe` or
 * `npm install -g @absterrg0/circe`, and even a CLI installed that way pins an
...
/**
 * Installs the circe release archive for `version` into the pinned runtime
...
const installFromArchive = Effect.fn("cloud.pinned_runtime.install_archive")(function* (
  ...
  if (platformKey === undefined) {
    return yield* new PinnedRuntimeInstallError({
      step: `selecting a circe release archive for ${input.platform}-${input.arch}`,
    });
  }
  ...
  const checksums = parseChecksums(
    new TextDecoder().decode(
      yield* fetchReleaseAsset(
        ...
        "downloading the circe release checksums",
      ),
    ),
  );
  ...
  if (expected === undefined) {
    return yield* new PinnedRuntimeInstallError({
      step: `finding ${fileName} in the circe release checksums`,
    });
  }
  const archive = yield* fetchReleaseAsset(
    httpClient,
    `${baseUrl}/${fileName}`,
    "downloading the circe release archive",
    input.onProgress,
  );
  ...
  const digest = yield* Effect.tryPromise({
    try: () => crypto.subtle.digest("SHA-256", archive),
      ...
      new PinnedRuntimeInstallError({ step: "verifying the circe release archive", cause }),
  });
  if (Encoding.encodeHex(new Uint8Array(digest)) !== expected) {
    return yield* new PinnedRuntimeInstallError({
      step: "verifying the circe release archive checksum",
    });
  }
  ...
  yield* fs
    ...
    .pipe(
      Effect.mapError(
        (cause) =>
          new PinnedRuntimeInstallError({ step: "writing the circe release archive", cause }),
      ),
    );
  input.onProgress?.({ stage: "extract" });
  const extractStep = "extracting the circe release archive";
  // The archive wraps everything in one directory named after its stem;
  // strip it so the executable lands at <versionDir>/circe.
  yield* input.runner
    .run({
