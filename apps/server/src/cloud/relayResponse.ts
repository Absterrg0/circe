...
import { RelayProtectedError } from "@circe/contracts/relay";
...
export function relayRequestError(cause: unknown) {
  return isRelayResponseError(cause)
    ...
    : new EnvironmentHttpInternalServerError({
        message: `Could not complete the Circe Mesh relay request. ${isHttpClientError(cause) ? `The relay request failed (${cause.reason._tag}).` : "The relay returned an unexpected response."} Check this machine's network connection and relay availability, then retry.`,
      });
}
