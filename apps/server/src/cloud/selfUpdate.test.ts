...
import { ServerSelfUpdateError, ThreadId } from "@circe/contracts";
    ...
    Effect.gen(function* () {
      if (request.url.endsWith("/SHA256SUMS")) {
        ...
        return HttpClientResponse.fromWeb(
          request,
          new Response(`${hex}  circe-1.1.0-linux-x64.tar.gz\n`),
        );
      }
      order.push("download");
    ...
    run: (input) =>
      Effect.gen(function* () {
        if (input.command === "tar") {
          ...
          yield* fs
            .writeFileString(path.join(stagingDir, "circe"), "#!/bin/sh\n")
            .pipe(Effect.orDie);
          return {
            stdout: "",
            stderr: "",
...
it.layer(NodeServices.layer)("server self update", (it) => {
  ...
  it.effect("rejects invalid versions and desktop-managed servers before staging", () =>
    Effect.gen(function* () {
      ...
      ).toBe("'latest' is not an exact Circe version.");
