...
>()("@absterrg0/circe/cloud/selfUpdate/ServerSelfUpdate") {}
...
export const make = Effect.fn("cloud.server_self_update.make")(function* () {
  ...
  const update: ServerSelfUpdate["Service"]["update"] = Effect.fn(
    ...
    if (capability === null) {
      return yield* failWith(
        "Remote updates require the Circe background service. Run `circe service install` on the server machine.",
      );
    }

    const targetVersion = input.targetVersion.trim();
    if (!isExactServiceVersion(targetVersion)) {
      return yield* failWith(`'${targetVersion}' is not an exact Circe version.`);
    }
      ...
      const paths = yield* ensurePinnedRuntimeInstalled({
        ...
        Effect.mapError((error) =>
          error._tag === "PinnedRuntimePreflightBlockedError"
            ...
            : failWith(`Could not prepare @absterrg0/circe@${targetVersion}.`, error),
        ),
      );
