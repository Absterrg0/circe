...
import { hasProjectSettingsOverrides, resolveProjectSettings } from "@circe/shared/projectSettings";
...
import { GitManagerError, GitPullRequestMaterializationError } from "@circe/contracts";
...
import type { GitManagerServiceError } from "@circe/contracts";
import * as GitVcsDriver from "../vcs/GitVcsDriver.ts";
import * as SourceControlProviderRegistry from "../sourceControl/SourceControlProviderRegistry.ts";
import { detectPrTemplate } from "../sourceControl/PrTemplateDetection.ts";
import type { ChangeRequest } from "@circe/contracts";
...
>()("@absterrg0/circe/git/GitManager") {}
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `circe/pr-${pullRequest.number}/${suffix}`;
}
...
export const make = Effect.gen(function* () {
  ...
  const runPrStep = Effect.fn("runPrStep")(function* (
    ...
    const bodyFile = path.join(
      tempDir,
      `circe-pr-body-${process.pid}-${yield* randomUUIDv4(cwd)}.md`,
    );
