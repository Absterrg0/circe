...
describe("linkCreatedPullRequest", () => {
    ...
    Effect.gen(function* () {
      ...
      yield* linkCreatedPullRequest({
        ...
        result: prResult({
          status: "created",
          number: 42,
          url: "https://github.com/t3tools/circe/pull/42",
        }),
        commandId,
  ...
  it.effect("dispatches nothing when the action produced no pull request", () =>
    Effect.gen(function* () {
      ...
      yield* linkCreatedPullRequest({
        threadId: THREAD_ID,
        result: prResult({ status: "created", url: "https://github.com/t3tools/circe/pull/42" }),
        commandId,
      }).pipe(Effect.provide(dependencies));
  ...
  it.effect("swallows an already-linked rejection and other dispatch failures", () =>
    Effect.gen(function* () {
      ...
      const result = prResult({
        ...
        number: 7,
        url: "https://github.com/t3tools/circe/pull/7",
      });
