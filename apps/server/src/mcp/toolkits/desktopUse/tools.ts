...
/**
 * The provider hands a desktop goal to the node's TypeSafe loop instead of
 * clicking step by step itself. The loop selects among grounded accessibility
 * elements and performs each action, so the provider never supplies a
 * coordinate or a selector.
 */
