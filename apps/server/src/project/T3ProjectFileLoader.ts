...
import { CIRCE_PROJECT_FILE_NAME, type T3ProjectFile } from "@circe/contracts";
import { T3ProjectFileFromJson } from "@circe/shared/t3ProjectFile";
...
export class T3ProjectFileLoadError extends Schema.TaggedError<T3ProjectFileLoadError>()(
  ...
  override get message(): string {
    return `Failed to ${this.operation} ${CIRCE_PROJECT_FILE_NAME} at ${this.filePath}.`;
  }
}
...
>()("@absterrg0/circe/project/T3ProjectFileLoader") {}
...
export const make = Effect.gen(function* () {
  ...
  const load: T3ProjectFileLoader["Service"]["load"] = Effect.fn("T3ProjectFileLoader.load")(
    function* (workspaceRoot) {
      const filePath = path.join(workspaceRoot, CIRCE_PROJECT_FILE_NAME);
