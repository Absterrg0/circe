...
import { AntigravitySettings, ProviderDriverKind, ProviderSetupError } from "@circe/contracts";
import { HostProcessPlatform } from "@circe/shared/hostProcess";
import {
  NodeRuntimeUnavailableError,
  nodeRuntimeUnavailableMessage,
} from "@circe/shared/nodeRuntime";
    ...
    Effect.gen(function* () {
      ...
      const makeDisposableRuntime = Effect.fn("AntigravityDriver.makeDisposableRuntime")(function* (
        ...
        const runtime = yield* makeRuntime({
          cwd,
          clientInfo: { name: "circe-provider-setup", version: "0.0.0" },
          mcpServers: [],
      ...
      const textGeneration = yield* makeAntigravityTextGeneration({
        ...
        makeRuntime: (cwd) =>
          makeRuntime({
            cwd,
            clientInfo: { name: "circe-text", version: "0.0.0" },
            mcpServers: [],
          }),
      });
