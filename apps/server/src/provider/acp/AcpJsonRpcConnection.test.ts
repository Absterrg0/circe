...
describe("AcpSessionRuntime", () => {
  ...
  it.effect("awaits native resume instead of using the load replay idle fallback", () =>
    Effect.gen(function* () {
      ...
      const runtime = yield* AcpSessionRuntime.make({
        ...
        spawn: {
          ...mockRuntimeOptions.spawn,
          env: { CIRCE_ACP_WAIT_FOR_RESUME_RELEASE: "1", CIRCE_ACP_SESSION_LIFECYCLE: "1" },
        },
        resumeSessionId: "mock-session-1",
        resumeMethod: "resume",
  ...
  it.effect("waits for native cancellation and drains final updates before another prompt", () =>
    Effect.gen(function* () {
      ...
      const runtime = yield* AcpSessionRuntime.make({
        ...
        spawn: {
          ...mockRuntimeOptions.spawn,
          env: { CIRCE_ACP_COMPLETE_FIRST_PROMPT_ON_CANCEL: "1" },
        },
  ...
  it.effect("fails a pending request when the stderr handler rejects the runtime", () =>
    Effect.gen(function* () {
      ...
      const runtime = yield* AcpSessionRuntime.make({
        ...mockRuntimeOptions,
        spawn: { ...mockRuntimeOptions.spawn, env: { CIRCE_ACP_FLOOD_STDERR: "1" } },
  ...
  it.effect("does not restore ambient variables to a sanitized child environment", () =>
    Effect.gen(function* () {
      yield* Effect.acquireRelease(
        Effect.sync(() => {
          const previous = process.env.CIRCE_ACP_RUNTIME_AMBIENT;
          process.env.CIRCE_ACP_RUNTIME_AMBIENT = "sentinel";
          return previous;
        }),
        (previous) =>
          Effect.sync(() => {
            if (previous === undefined) delete process.env.CIRCE_ACP_RUNTIME_AMBIENT;
            else process.env.CIRCE_ACP_RUNTIME_AMBIENT = previous;
          }),
      );
      const runtime = yield* AcpSessionRuntime.make({
        ...mockRuntimeOptions,
        spawn: {
          ...
          args: mockAgentArgs,
          extendEnv: false,
          env: { CIRCE_ACP_RUNTIME_EXPLICIT: "kept" },
        },
      });
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          ...
          spawn: {
            command: mockAgentCommand,
            args: mockAgentArgs,
            env: { CIRCE_ACP_AUTH_METHOD_ID: "test" },
          },
          cwd: process.cwd(),
          clientInfo: { name: "t3-test", version: "0.0.0" },
          requestLogger: (event) =>
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          ...
          spawn: {
            ...
            args: mockAgentArgs,
            env: {
              CIRCE_ACP_AUTH_METHOD_ID: "test",
              CIRCE_ACP_REQUIRE_AUTH: "1",
            },
          },
          cwd: process.cwd(),
          clientInfo: { name: "t3-test", version: "0.0.0" },
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          spawn: {
            ...
            args: mockAgentArgs,
            env: {
              CIRCE_ACP_EMIT_FOREIGN_SESSION_UPDATES: "1",
            },
          },
          cwd: process.cwd(),
          clientInfo: { name: "t3-test", version: "0.0.0" },
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          spawn: {
            ...
            args: mockAgentArgs,
            env: {
              CIRCE_ACP_HANG_FIRST_PROMPT_FOREVER: "1",
            },
          },
          cwd: process.cwd(),
          clientInfo: { name: "t3-test", version: "0.0.0" },
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          spawn: {
            ...
            args: mockAgentArgs,
            env: {
              CIRCE_ACP_EMIT_INTERLEAVED_ASSISTANT_TOOL_CALLS: "1",
            },
          },
          cwd: process.cwd(),
          clientInfo: { name: "t3-test", version: "0.0.0" },
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          spawn: {
            ...
            args: mockAgentArgs,
            env: {
              CIRCE_ACP_EMIT_GENERIC_TOOL_PLACEHOLDERS: "1",
            },
          },
          cwd: process.cwd(),
          clientInfo: { name: "t3-test", version: "0.0.0" },
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          ...
          spawn: {
            ...
            args: mockAgentArgs,
            env: {
              CIRCE_ACP_FAIL_LOAD_SESSION_AFTER_CONFIG_REPLAY: "1",
            },
          },
          cwd: process.cwd(),
          clientInfo: { name: "t3-test", version: "0.0.0" },
    ...
    Effect.gen(function* () {
      ...
      const runtime = yield* AcpSessionRuntime.make({
        ...
        spawn: {
          ...
          args: mockAgentArgs,
          env: {
            CIRCE_ACP_DELAY_LOAD_SESSION_AFTER_REPLAY: "1",
            CIRCE_ACP_LOAD_SESSION_DELAY_MS: "250",
          },
        },
        cwd: process.cwd(),
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          ...
          spawn: {
            ...
            args: mockAgentArgs,
            env: {
              CIRCE_ACP_HANG_LOAD_SESSION_AFTER_REPLAY: "1",
              CIRCE_ACP_LOAD_SESSION_DELAY_MS: "10000",
            },
          },
          cwd: process.cwd(),
          resumeSessionId: "mock-session-1",
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          ...
          spawn: {
            ...
            args: mockAgentArgs,
            env: {
              CIRCE_ACP_REQUEST_LOG_PATH: requestLogPath,
            },
          },
          cwd: process.cwd(),
          clientInfo: { name: "t3-test", version: "0.0.0" },
