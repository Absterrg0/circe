...
import { PullRequestOperationError } from "@circe/contracts";
...
it.effect("reads nothing from a host with no implementation, but reports it", () =>
  Effect.gen(function* () {
    ...
    const service = yield* makeService({
      projects: [
        project({ id: "p1", title: "t3code", workspaceRoot: "/a", repository: "Absterrg0/circe" }),
    ...
    assert.deepStrictEqual(listed, ["Absterrg0/circe"]);
      ...
      providers: [
        fakeProvider("github", {
          listChangeRequests: ({ repository }) =>
            Effect.succeed({
              items: [changeRequest(1, "2026-07-02T00:00:00Z")],
              truncated: repository === "Absterrg0/circe",
              continues: true,
            }),
        }),
      ],
...
it.effect("reads a host's repositories in one search, and files the rows back under each", () =>
  Effect.gen(function* () {
          ...
          listChangeRequestsAcross: (input) => {
            ...
            return Effect.succeed({
              items: [
                ...
                batchedChangeRequest(2, "Absterrg0/circe", "2026-07-02T00:00:00Z"),
              ],
              truncated: false,
            });
          },
    ...
    assert.deepStrictEqual(asked, [["Absterrg0/circe", "acme/web"]]);
