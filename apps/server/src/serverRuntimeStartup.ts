...
>()("@absterrg0/circe/serverRuntimeStartup") {}
...
const make = (options?: StartupOptions) =>
  Effect.gen(function* () {
    ...
    // Circe-owned reactors (follow-up dispatch, push notifications) outlive
    // individual startup phases and stop with the runtime.
    ...
    const startup = Effect.gen(function* () {
      ...
      // Circe-owned reactors start after the V2 effect worker is live.
      yield* runStartupPhase(
        "circe.reactors.start",
        Effect.gen(function* () {
