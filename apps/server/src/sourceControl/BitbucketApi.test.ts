...
import { GitCommandError } from "@circe/contracts";
...
const bitbucketPullRequest = {
  ...
  links: {
    html: {
      href: "https://bitbucket.org/Absterrg0/circe/pull-requests/42",
    },
  },
  source: {
...
const repositoryJson = {
  full_name: "Absterrg0/circe",
  links: {
    html: { href: "https://bitbucket.org/Absterrg0/circe" },
    clone: [
      { name: "https", href: "https://bitbucket.org/Absterrg0/circe.git" },
      { name: "ssh", href: "git@bitbucket.org:Absterrg0/circe.git" },
    ],
  },
...
function makeLayer(input: {
  ...
  const gitMock = {
    readConfigValue: vi.fn<GitVcsDriver.GitVcsDriver["Service"]["readConfigValue"]>(() =>
      Effect.succeed<string | null>("git@bitbucket.org:Absterrg0/circe.git"),
    ),
    ...
    listRemotes: () =>
      Effect.succeed({
        remotes: [
          {
            name: "origin",
            url: "git@bitbucket.org:Absterrg0/circe.git",
            pushUrl: Option.none(),
            isPrimary: true,
          },
        ],
...
it.effect("parses pull request responses from the Bitbucket REST API", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(result, {
      number: 42,
      title: "Add Bitbucket provider",
      url: "https://bitbucket.org/Absterrg0/circe/pull-requests/42",
      baseRefName: "main",
    ...
    assert.strictEqual(
      ...
      "https://api.test.local/2.0/repositories/Absterrg0/circe/pullrequests/42",
    );
...
it.effect("lists pull requests with Bitbucket state and source branch query params", () => {
        ...
        values: [
          {
            ...
            source: {
              ...
              repository: { full_name: "Absterrg0/circe" },
            },
          },
        ],
      }),
  ...
  return Effect.gen(function* () {
    ...
    assert.strictEqual(
      request?.url,
      "https://api.test.local/2.0/repositories/Absterrg0/circe/pullrequests",
    );
...
it.effect("reads repository clone URLs and default branch", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(cloneUrls, {
      nameWithOwner: "Absterrg0/circe",
      url: "https://bitbucket.org/Absterrg0/circe.git",
      sshUrl: "git@bitbucket.org:Absterrg0/circe.git",
    });
  ...
  () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        [
          "https://api.test.local/2.0/repositories/Absterrg0/circe",
          "https://api.test.local/2.0/repositories/Absterrg0/circe/branching-model",
        ].toSorted(),
      );
    }).pipe(Effect.provide(layer));
  },
...
it.effect("creates repositories through the Bitbucket REST API", () => {
  ...
  return Effect.gen(function* () {
    ...
    const request = execute.mock.calls[0]?.[0];
    assert.strictEqual(request?.url, "https://api.test.local/2.0/repositories/Absterrg0/circe");
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(JSON.parse(new TextDecoder().decode(rawBody)), {
      ...
      source: {
        ...
        repository: { full_name: "owner/circe" },
      },
      destination: {
        branch: { name: "main" },
      },
  ...
  return Effect.gen(function* () {
    ...
    yield* bitbucket.checkoutPullRequest({
      ...
      context: {
        ...
        remoteName: "origin",
        remoteUrl: "git@bitbucket.org:Absterrg0/circe.git",
      },
      reference: "42",
      force: true,
    });
...
it.effect("checks out fork pull requests through an ensured fork remote", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(git.fetchRemoteBranch.mock.calls[0]?.[0], {
      ...
      remoteName: "octocat",
      remoteBranch: "main",
      localBranch: "circe/pr-42/main",
    });
    assert.deepStrictEqual(git.setBranchUpstream.mock.calls[0]?.[0], {
      cwd: "/repo",
      branch: "circe/pr-42/main",
      remoteName: "octocat",
      remoteBranch: "main",
    });
    assert.deepStrictEqual(git.switchRef.mock.calls[0]?.[0], {
      cwd: "/repo",
      refName: "circe/pr-42/main",
    });
  }).pipe(Effect.provide(layer));
});
