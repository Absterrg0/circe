...
import { GrokSettings, ProviderInstanceId, TextGenerationError } from "@circe/contracts";
...
it.layer(GrokTextGenerationTestLayer)("GrokTextGeneration", (it) => {
  it.effect("refuses structured generation when ACP cannot guarantee a tool-free session", () =>
    withFakeAcpGrok(
      {
        CIRCE_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({ action: "status" }),
      },
      (textGeneration) =>
        Effect.gen(function* () {
    ...
    return withFakeAcpGrok(
      {
        ...
        CIRCE_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({
  ...
  it.effect("extracts the JSON object when Grok wraps it in conversational text", () =>
    withFakeAcpGrok(
      {
        CIRCE_ACP_PROMPT_RESPONSE_TEXT:
  ...
  it.effect("surfaces ACP request failures as text generation errors", () =>
    withFakeAcpGrok(
      {
        CIRCE_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({ branch: "unreachable" }),
      },
      (textGeneration) =>
        Effect.gen(function* () {
  ...
  it.effect("fails with TextGenerationError when output is unparseable JSON", () =>
    withFakeAcpGrok(
      {
        CIRCE_ACP_PROMPT_RESPONSE_TEXT: "totally not json output from a confused model",
      },
      (textGeneration) =>
        Effect.gen(function* () {
          const error = yield* Effect.flip(
