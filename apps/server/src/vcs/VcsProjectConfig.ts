...
import { VcsDriverKind, type VcsDriverKind as VcsDriverKindType } from "@circe/contracts";
import { fromLenientJson } from "@circe/shared/schemaJson";
...
>()("@absterrg0/circe/vcs/VcsProjectConfig") {}
...
export const make = Effect.gen(function* () {
  ...
  const findConfigPath = Effect.fn("VcsProjectConfig.findConfigPath")(function* (cwd: string) {
    let current = cwd;
    while (true) {
      const candidate = path.join(current, ".circe", "vcs.json");
