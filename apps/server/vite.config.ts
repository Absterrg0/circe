...
const packExecutable = process.env.CIRCE_PACK_EXE === "1";
...
const packExecutableTarget = process.env.CIRCE_PACK_EXE_TARGET?.trim();
if (packExecutableTarget && !Object.hasOwn(SEA_TARGETS, packExecutableTarget)) {
  throw new Error(
    `CIRCE_PACK_EXE_TARGET must be one of ${Object.keys(SEA_TARGETS).join(", ")}, got "${packExecutableTarget}".`,
  );
}
...
export default mergeConfig(
  ...
  defineConfig({
    ...
    pack: {
      ...
      define: {
        __CIRCE_BUILD_CHANNEL__: JSON.stringify(cliBuildChannel),
        __CIRCE_BUILD_RELAY_URL__: JSON.stringify(repoEnv.CIRCE_RELAY_URL?.trim() ?? ""),
        __CIRCE_BUILD_CLERK_PUBLISHABLE_KEY__: JSON.stringify(
          repoEnv.CIRCE_CLERK_PUBLISHABLE_KEY?.trim() ?? "",
        ),
        __CIRCE_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__: JSON.stringify(
          repoEnv.CIRCE_CLERK_CLI_OAUTH_CLIENT_ID?.trim() ?? "",
        ),
        __CIRCE_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__: JSON.stringify(
          repoEnv.CIRCE_RELAY_CLIENT_OTLP_TRACES_URL?.trim() ?? "",
        ),
        __CIRCE_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__: JSON.stringify(
          repoEnv.CIRCE_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() ?? "",
        ),
        __CIRCE_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__: JSON.stringify(
          repoEnv.CIRCE_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() ?? "",
        ),
      },
    },
    test: {
