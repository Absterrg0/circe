...
/**
 * The user's Circe Mesh environments from relay discovery, each with a
  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `Connecting to ${environment.label} through Circe Mesh.`,
      });
      return true;
    }
    ...
    const message =
      cause instanceof Error ? cause.message : "Could not connect the Circe Mesh environment.";
