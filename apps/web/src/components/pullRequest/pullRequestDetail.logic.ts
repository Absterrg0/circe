...
export function pullRequestCheckoutCommand(
  ...
  switch (provider) {
    ...
    case "bitbucket": {
      ...
      return `git clone --single-branch --branch ${headBranch} https://bitbucket.org/${headRepositoryNameWithOwner}.git circe-pr-${number}`;
    }
    case "unknown":
      return null;
  }
...
const pullRequestDetailSnapshotKey = (
  ...
  reference.host
    ? `circe.pullRequests.detail:${JSON.stringify([environmentId, reference.projectId, reference.host.toLowerCase(), reference.repository.toLowerCase(), reference.number])}`
    : `circe.pullRequests.detail:${environmentId}:${reference.projectId}:${reference.repository}#${reference.number}`;
