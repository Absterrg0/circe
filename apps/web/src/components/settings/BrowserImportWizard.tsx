...
import type { BrowserImportSource } from "@circe/contracts";
import { BROWSER_IMPORT_FAILURE_COPY } from "@circe/contracts";
...
function FullDiskAccessStep({
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>Let Circe read {source.name}&rsquo;s cookies</DialogTitle>
        <DialogDescription>
          To import cookies from {source.name}, Circe needs Full Disk Access. Turn it on in System
      ...
      <DialogPanel>
        ...
        {!permission.isReady(["fullDiskAccess"]) ? (
          <p className="mt-3 text-xs text-muted-foreground">
            {stillRequired
              ? "Access is still required. Quit and reopen Circe if you just allowed it, then retry the import."
              : "If access doesn't update after you allow it, quit and reopen Circe, then retry the import."}
          </p>
        ) : null}
      </DialogPanel>
      <DialogFooter>
