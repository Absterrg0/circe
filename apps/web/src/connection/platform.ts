...
import { bootstrapRemoteBearerSession } from "@circe/client/authorization";
import { fetchRemoteEnvironmentDescriptor } from "@circe/client/environment";
import { managedRelayAccountChanges, managedRelaySessionAtom } from "@circe/client/relay";
import { EnvironmentRpcRequestObserver } from "@circe/client/rpc";
import {
    ...
    const cloudSession = CloudSession.of({
      ...
      clerkToken: Effect.gen(function* () {
        ...
        if (session === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "Sign in to Circe Mesh to connect this environment.",
          });
        }
        ...
        if (token === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "The Circe Mesh session is unavailable.",
          });
        }
        return token;
      }),
