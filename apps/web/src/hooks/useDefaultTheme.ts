...
/**
 * Scoped per environment: each machine's `circe theme set` is its own act, so
 * hopping between primary environments neither replays one environment's
 * theme over the user's pick nor swallows another's.
 */
const APPLIED_DEFAULT_THEME_STORAGE_PREFIX = "circe:default-theme-applied:v2:";
...
/**
 * Applies the environment's theme (`circe theme set <id>`). Each set switches
