...
describe("parseChangeRequestUrl", () => {
  it("reads a GitHub pull request", () => {
    expect(parseChangeRequestUrl("https://github.com/T3Tools/circe/pull/123")).toEqual({
      host: "github.com",
      repository: "t3tools/circe",
      number: 123,
    });
  });
  ...
  it("reads a GitLab merge request, nested groups and all", () => {
    expect(
      parseChangeRequestUrl("https://gitlab.com/t3tools/platform/circe/-/merge_requests/42"),
    ).toEqual({
      host: "gitlab.com",
      repository: "t3tools/platform/circe",
      number: 42,
    });
  });
  ...
  it("reads both Azure DevOps URL forms, keeping `_git` in the repository path", () => {
    expect(
      parseChangeRequestUrl("https://dev.azure.com/acme/platform/_git/circe/pullrequest/17"),
    ).toEqual({
      host: "dev.azure.com",
      repository: "acme/platform/_git/circe",
      number: 17,
    });
    expect(
      parseChangeRequestUrl("https://acme.visualstudio.com/platform/_git/circe/pullrequest/17"),
    ).toEqual({
      host: "acme.visualstudio.com",
      repository: "platform/_git/circe",
      number: 17,
    });
  });

  it("survives trailing segments, a trailing slash and a query string", () => {
    expect(parseChangeRequestUrl("https://github.com/t3tools/circe/pull/123/files?w=1")).toEqual({
      host: "github.com",
      repository: "t3tools/circe",
      number: 123,
    });
  ...
  it("claims nothing it cannot be sure of, so the link goes to the browser", () => {
    for (const link of [
      "https://github.com/t3tools/circe/issues/123",
      "https://github.com/t3tools/circe/commit/0a1b2c3",
      "https://github.com/t3tools/circe",
      "https://github.com/t3tools/circe/pull/abc",
      "https://gitlab.com/t3tools/circe/-/snippets/12",
      "https://gitlab.com/t3tools/circe/-/issues/12",
      ...
      "javascript:alert(1)//github.com/t3tools/circe/pull/1",
      "not a url",
    ]) {
...
describe("findProjectForChangeRequest", () => {
  ...
  it("matches a nested GitLab group by the whole path below the host", () => {
    ...
    // two-segment owner/name form would look for `t3tools/circe` and find nothing.
    const projects = [
      project({
        canonicalKey: "gitlab.com/t3tools/platform/circe",
        provider: "gitlab",
        displayName: "t3tools/platform/circe",
        owner: "t3tools",
        name: "circe",
      }),
    ];
