...
import { DEV_PROXIED_PATH_PREFIXES } from "@circe/shared/devProxy";
...
const isSingleOriginDev = process.env.CIRCE_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_CIRCE_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.CIRCE_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: CIRCE_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); CIRCE_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.CIRCE_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // CIRCE_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.CIRCE_PORT, configuredWsUrl);
...
const configuredAllowedHosts = (process.env.CIRCE_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_CIRCE_RELAY_URL": JSON.stringify(configuredRelayUrl),
