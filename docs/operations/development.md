...
Linked worktrees default to their own `.t3/userdata`, even when `CIRCE_HOME` is set.
...
but occupied ports can shift them. `CIRCE_PORT_OFFSET` or `CIRCE_DEV_INSTANCE` can select a
...
`CIRCE_BUNDLED_DEV=0` opts out when debugging bundler differences. Two reload traps matter
...
```dotenv
CIRCE_DEV_AUTH_TOKEN=<the value generated above>
```
...
```sh
export CIRCE_DEV_AUTH_TOKEN="<the value generated above>"
```
