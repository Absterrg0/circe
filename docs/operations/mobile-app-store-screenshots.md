...
the software keyboard, and supplies deterministic Circe Mesh discovery rows to the real
...
ARM64 by default for local Apple Silicon development; CI sets `CIRCE_SHOWCASE_ANDROID_ABI=x86_64` so the
...
`pnpm --filter @circe/mobile showcase` starts Expo on its normal port, so it is not compatible with
