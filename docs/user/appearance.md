...
```bash
circe theme set nightfall
```

Clients that are offline apply it when they reconnect. Each client applies the setting once;
choosing another theme afterward sticks until the next `circe theme set`. Run the command again to
reapply it, even if the name is unchanged.

`circe theme clear` removes the default without changing anyone's current theme. `circe theme show` lists
...
can be selected with `circe theme set nightfall`. Keep the filename stable when updating its colors.
