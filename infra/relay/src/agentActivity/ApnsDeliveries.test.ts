...
const config = RelayConfiguration.RelayConfiguration.of({
  ...
  apns: {
    ...
    bundleId: "com.abstergo.circe.dev",
  },
  ...
  it.effect("queues Live Activity jobs with the device's APNs routing", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* deliveries.sendForTarget({
        target: {
          ...target,
          bundle_id: "com.abstergo.circe.preview",
      ...
      expect(queuedJobs).toMatchObject([
        {
          payload: {
            ...
            target: {
              token: "activity-token",
              bundleId: "com.abstergo.circe.preview",
  ...
  it.effect("sends signed jobs to the device's APNs environment and bundle topic", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(requests[0]?.headers["apns-topic"]).toBe(
        "com.abstergo.circe.preview.push-type.liveactivity",
      );
    }).pipe(
      Effect.provide(
        makeLayer({
          attempts,
          currentTargets: [
            { ...target, bundle_id: "com.abstergo.circe.preview", aps_environment: "sandbox" },
          ],
    ...
    for (const changed of ["bundle", "environment", "legacy"] as const) {
      it.effect(`routes ${kind} using current registration with ${changed} job metadata`, () => {
        ...
        const payload = makeApnsDeliveryJobPayload({
          ...
          ...(changed === "legacy"
            ? {}
            : { bundleId: "com.abstergo.circe.dev", apsEnvironment: "sandbox" as const }),
        ...
        return Effect.gen(function* () {
          ...
          expect(requests[0]?.headers["apns-topic"]).toBe(
            `${changed === "bundle" ? "com.abstergo.circe.preview" : "com.abstergo.circe.dev"}${kind === "live_activity_update" ? ".push-type.liveactivity" : ""}`,
          );
        }).pipe(
          Effect.provide(
            makeLayer({
              ...
              currentTargets: [
                {
                  ...
                  bundle_id:
                    changed === "bundle" ? "com.abstergo.circe.preview" : "com.abstergo.circe.dev",
