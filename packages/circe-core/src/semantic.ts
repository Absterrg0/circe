...
const CIRCE_SEMANTIC_PROMPT_RULES: ReadonlyArray<string> = [
  ...
  "Quick actions take no project or task. A weather or local-time question uses action lookup with lookup {kind: weather|time, location, day: now|today|tomorrow}. Copy location verbatim from the user's words without translating or normalizing names, and use day now unless the user says today or tomorrow. A request to open a site uses action open-website with website set to the named site or URL the user gave. Never use lookup or open-website for work that edits, deploys, or investigates a project; those are ordinary start tasks. A request that combines a lookup or website launch with any other work is unsupported: quick actions never take refs and never combine. A request that needs several steps inside the user's own browser (find something, fill it in, submit it) uses action browse with browserGoal set to the user's own instruction to the same effect and no refs; the origin client starts it directly and the node grounds every step in the user's real browser. A request that needs several steps in the shared in-app preview, only when the user asks for the preview, localhost, or a dev server, uses action preview with browserGoal set to the user's own instruction to the same effect and no refs; the origin client starts it directly and the node grounds every step in the preview. A request that needs several steps on this machine's own desktop (open an app, fill a form in it, click through a dialog) uses action computer with computerGoal set to the user's own instruction to the same effect and no refs; the origin client confirms once before it starts, and the node grounds every step over accessibility elements. The host rejects a location that does not appear in the transcript and a website outside the allowed set.",
