...
import { RelayEnvironmentConnectScope, RelayEnvironmentStatusScope } from "@circe/contracts/relay";
import { decodeRelayJwt } from "@circe/shared/relayJwt";
      ...
      return yield* Effect.tryPromise({
        try: () => readCachedClerkToken(nowMillis),
          ...
          new ManagedRelaySessionError({
            message: "Could not obtain the Circe Mesh session token.",
            cause,
          }),
      });
    }),
  ...
  return session.readClerkToken().pipe(
    Effect.flatMap((token) =>
      token
        ...
        : Effect.fail(
            new ManagedRelaySessionError({
              message: "The Circe Mesh session token is unavailable.",
            }),
          ),
    ),
  );
...
export const deregisterManagedRelayEnvironment = Effect.fn(
  ...
  if (!session || session.accountId !== input.accountId) {
    return yield* new ManagedRelaySessionError({
      message: "Sign in to Circe Mesh before deregistering an environment.",
    });
  }
...
export const setManagedRelayEnvironmentEnabled = Effect.fn(
  ...
  if (!session || session.accountId !== input.accountId) {
    return yield* new ManagedRelaySessionError({
      message: "Sign in to Circe Mesh before changing device access.",
    });
  }
  const clerkToken = yield* readSessionClerkToken(session);
  const relay = yield* ManagedRelay.ManagedRelayClient;
...
function requireClerkToken(
  ...
  if (!session || session.accountId !== accountId) {
    return Effect.fail(
      new ManagedRelaySessionError({
        message: "Sign in to Circe Mesh before loading relay data.",
      }),
    );
  }
    ...
    error =
      isManagedRelayRequestFailedError(cause) && cause.relayError
        ...
        : cause instanceof Error
          ? cause.message
          : "Could not load Circe Mesh data.";
