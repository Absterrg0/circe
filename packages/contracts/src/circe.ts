...
/**
 * A bounded action the origin client performs on the user's device. The node
 * never attempts it: it returns the tool, its revalidated arguments, and the
 * acceptance speech. The client runs its executor, speaks the real result, and
 * reports failure only from its own execution, never because the capability
 * was missing (it would not have been offered).
 */
...
/**
 * Stop one running node mission (browser or computer use). `requestId` is the
 * client request id the mission registered under, or `thread:<id>` for a
 * provider-delegated desktop goal.
 */
...
/** `cancelled` is false when no live mission held the id and nothing stopped. */
