...
/**
 * The environment's theme, set with `circe theme set <id>`. Each client applies
...
// Deliberately absent from ServerSettingsPatch: `circe theme set` checks that an
...
export const OpenCodeSettings = makeProviderSettingsSchema(
  {
    ...
    serverUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Server URL",
        description: "Leave blank to let Circe spawn the server when needed.",
        providerSettingsForm: {
...
export const ServerSettings = Schema.Struct({
   ...
   * withholds the MCP credential, so the `circe` server (and with it every
   ...
   * of the same value from one they already applied: `circe theme set` must act
