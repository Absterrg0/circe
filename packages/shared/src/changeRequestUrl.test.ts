...
describe("parseChangeRequestUrl", () => {
  ...
  it("reads a GitLab merge request on any host, nested groups and all", () => {
    ...
    ).toEqual({ host: "gitlab.com", repository: "t3tools/platform/circe", number: 42 });
  ...
  it("reads Bitbucket and both Azure DevOps URL forms", () => {
    ...
    ).toEqual({ host: "dev.azure.com", repository: "acme/platform/_git/circe", number: 17 });
    expect(
      parseChangeRequestUrl("https://acme.visualstudio.com/platform/_git/circe/pullrequest/17"),
    ).toEqual({ host: "acme.visualstudio.com", repository: "platform/_git/circe", number: 17 });
  });
