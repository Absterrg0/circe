...
describe("connectAuth", () => {
  it("round-trips state, challenge, and loopback port through the authorize URL fragment", () => {
    const url = buildConnectAuthorizeRequestUrl({
      hostedAppUrl: "https://app.heycirce.com",
    ...
    expect(parsed.origin).toBe("https://app.heycirce.com");
  ...
  it("rejects authorize requests missing state, challenge, or port", () => {
    expect(readConnectAuthorizeRequest(new URL("https://app.heycirce.com/connect"))).toBeNull();
    expect(
      readConnectAuthorizeRequest(new URL("https://app.heycirce.com/connect#state=abc&port=34338")),
    ).toBeNull();
    expect(
      readConnectAuthorizeRequest(new URL("https://app.heycirce.com/connect#challenge=abc&port=34338")),
    ).toBeNull();
    expect(
      readConnectAuthorizeRequest(new URL("https://app.heycirce.com/connect#state=abc&challenge=abc")),
    ).toBeNull();
  });
  ...
  it("rejects authorize requests whose loopback port is corrupted", () => {
    for (const port of ["", "abc", "-1", "0", "65536", "34338x", "34 38"]) {
      const url = new URL(
        `https://app.heycirce.com/connect#state=state-1&challenge=challenge-1&port=${encodeURIComponent(port)}`,
      );
  ...
  it("builds a PKCE authorize URL against the Clerk endpoint", () => {
    const url = new URL(
      buildConnectClerkAuthorizeUrl({
        authorizationEndpoint: "https://clerk.heycirce.com/oauth/authorize",
    ...
    expect(url.origin).toBe("https://clerk.heycirce.com");
