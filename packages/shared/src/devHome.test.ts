...
import { resolveGitWorktreePath, resolveWorktreeCirceHome } from "./devHome.ts";
...
describe("resolveWorktreeCirceHome", () => {
  it.effect("answers with .t3 before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeCirceHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".circe"));
