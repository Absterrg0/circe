 ...
 * `~/.circe` that a user's installed Circe runs against.
 *
 * A linked git worktree gets its own (gitignored) `.circe`: feature work in a
 * throwaway branch must not share a database with the real app, and an ambient
 * `CIRCE_HOME` (or the deprecated `T3CODE_HOME` alias) counts as an explicit
 * base dir — flipping the state directory from `<base>/dev` to
 ...
 */
export const resolveWorktreeCirceHome = (
  cwd: string,
): Effect.Effect<string | undefined, never, FileSystem.FileSystem | Path.Path> =>
  Effect.gen(function* () {
    ...
    const path = yield* Path.Path;
    return path.join(worktreePath, ".circe");
  });
