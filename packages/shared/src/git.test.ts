import type { VcsStatusRemoteResult, VcsStatusResult } from "@circe/contracts";
...
describe("normalizeGitRemoteUrl", () => {
  it("canonicalizes equivalent GitHub remotes across protocol variants", () => {
    expect(normalizeGitRemoteUrl("git@github.com:T3Tools/circe.git")).toBe(
      "github.com/t3tools/circe",
    );
    expect(normalizeGitRemoteUrl("https://github.com/T3Tools/circe.git")).toBe(
      "github.com/t3tools/circe",
    );
    expect(normalizeGitRemoteUrl("ssh://git@github.com/T3Tools/circe")).toBe(
      "github.com/t3tools/circe",
    );
  });

  it("preserves nested group paths for providers like GitLab", () => {
    expect(normalizeGitRemoteUrl("git@gitlab.com:T3Tools/platform/circe.git")).toBe(
      "gitlab.com/t3tools/platform/circe",
    );
    expect(normalizeGitRemoteUrl("https://gitlab.com/T3Tools/platform/circe.git")).toBe(
      "gitlab.com/t3tools/platform/circe",
    );
  });
...
describe("parseOriginUrlFromGitConfig", () => {
  it("reads the origin url and ignores other remotes", () => {
    ...
    expect(parseOriginUrlFromGitConfig(config)).toBe("git@github.com:Absterrg0/circe.git");
  });
...
describe("parseGitHubRepositoryNameWithOwnerFromRemoteUrl", () => {
  it("extracts the owner and repository from common GitHub remote shapes", () => {
    expect(
      parseGitHubRepositoryNameWithOwnerFromRemoteUrl("git@github.com:T3Tools/circe.git"),
    ).toBe("T3Tools/circe");
    expect(
      parseGitHubRepositoryNameWithOwnerFromRemoteUrl("https://github.com/T3Tools/circe.git"),
    ).toBe("T3Tools/circe");
    expect(
      parseGitHubRepositoryNameWithOwnerFromRemoteUrl("ssh://github.com/T3Tools/circe.git"),
    ).toBe("T3Tools/circe");
  });
});
