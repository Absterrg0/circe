/**
 * `dist/bin.mjs` of the `@absterrg0/circe` npm package: the entry point
 * boot-service launchers installed before 0.0.41 run with Node to start a new
 * version they just npm-installed. It forwards everything (arguments, stdio,
 * the IPC channel the launcher talks over, signals, exit status) to the
 * platform executable in the sibling `@absterrg0/circe-<platform>-<arch>`
 * package.
 *
...
const executable = join(dirname(require.resolve("@absterrg0/circe-" + process.platform + "-" + process.arch + "/package.json")), executableName);
...
const fail = (error) => {
  if (!error) return;
  process.stderr.write("circe: " + error.message + "\\n");
  child.kill("SIGTERM");
  process.exitCode = 1;
};
if (ipc) {
