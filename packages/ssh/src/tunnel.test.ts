...
describe("ssh tunnel scripts", () => {
  it("installs and runs the release archive without Node, npm, or npx", () => {
    ...
    assert.include(script, "CIRCE_ARCHIVE_VERSION='1.2.3-preview.20260911.4'");
    assert.include(script, "CIRCE_NODE_SCRIPT_PATH=''");
    assert.include(
      script,
      "CIRCE_RELEASE_BASE_URL='https://github.com/Absterrg0/circe/releases/download'",
    );
    assert.include(script, 'CIRCE_RUNTIME_DIR="$HOME/.circe/runtime/versions/$CIRCE_ARCHIVE_VERSION"');
    assert.include(script, 'CIRCE_ARCHIVE="circe-$CIRCE_ARCHIVE_VERSION-$CIRCE_PLATFORM-$CIRCE_ARCH.tar.gz"');
    assert.include(script, "SHA256SUMS");
    assert.include(script, 'exec "$CIRCE_RUNTIME_DIR/circe" "$@"');
    assert.notInclude(script, "npx");
    assert.notInclude(script, "npm exec");
    assert.notInclude(script, "circe@latest");
    assert.notInclude(script, 'exec circe "$@"');
    ...
    assert.include(script, 'while ! mkdir "$CIRCE_LOCK" 2>/dev/null; do');
    assert.include(script, 'mv "$CIRCE_LOCK/pid.tmp" "$CIRCE_LOCK/pid"');
    assert.include(script, 'if ! kill -0 "$CIRCE_LOCK_OWNER" 2>/dev/null; then');
    assert.include(script, 'if [ "$CIRCE_LOCK_UNOWNED" -ge 5 ]; then');
    assert.include(script, 'if [ "$CIRCE_LOCK_WAITED" -ge 360 ]; then');
    assert.include(script, '"$CIRCE_STAGING/SHA256SUMS" 30');
    assert.include(script, '"$CIRCE_STAGING/$CIRCE_ARCHIVE" 240');
    assert.notInclude(script, "CIRCE_LOCK_CANDIDATE");
    assert.notInclude(script, "-mmin");
    assert.equal(script.split("if ! t3_runtime_ready; then").length - 1, 2);
    assert.isBelow(
      script.indexOf('"$CIRCE_STAGING/circe" --version'),
      script.indexOf('> "$CIRCE_STAGING/.install-complete"'),
    );
    ...
    assert.isBelow(
      ...
      script.indexOf('exec node "$CIRCE_NODE_SCRIPT_PATH" "$@"'),
    );
    assert.isBelow(
      script.indexOf('exec node "$CIRCE_NODE_SCRIPT_PATH" "$@"'),
      script.indexOf("CIRCE_ARCHIVE_VERSION="),
    );

    const launch = buildRemoteLaunchScript({
      ...ARCHIVE,
      releaseBaseUrl: "https://mirror.example/circe/",
    });
    assert.include(launch, "CIRCE_ARCHIVE_MODE=1");
    assert.include(launch, "CIRCE_RELEASE_BASE_URL='https://mirror.example/circe'");
    ...
    assert.include(buildRemoteLaunchScript(NODE_SCRIPT), "CIRCE_ARCHIVE_MODE=0");
  });
  ...
  it("rejects archive versions that are not a single exact version segment", () => {
    ...
    assert.include(
      ...
      "CIRCE_ARCHIVE_VERSION='1.2.3-preview.20260911.4'",
    );
  });
  ...
  it("does not hard-code a remote node engine range", () => {
    ...
    assert.include(script, "CIRCE_NODE_ENGINE_RANGE=''");
    assert.notInclude(script, TEST_NODE_ENGINE_RANGE);
  });

  it("builds the remote circe runner with a node script override", () => {
    ...
    assert.include(
      script,
      "CIRCE_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$CIRCE_NODE_SCRIPT_PATH" "$@"');
    assert.include(script, "CIRCE_ARCHIVE_VERSION=''");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `CIRCE_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for CIRCE_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
    assert.notInclude(script, "ensure $NVM_DIR/nvm.sh is available");
    assert.notInclude(script, "npx");
  });

  it("uses the remote circe runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(devLaunch, `CIRCE_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(launch, "CIRCE_ARCHIVE_VERSION='1.2.3-preview.20260911.4'");
    assert.include(
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "circe-archive-runner-" });
        ...
        for (const result of results) {
          ...
          assert.include(result.stdout, `circe v${archiveVersion}`);
        }
        const versionsDir = `${home}/.circe/runtime/versions`;
