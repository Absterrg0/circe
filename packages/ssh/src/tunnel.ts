import type { DesktopSshEnvironmentBootstrap, DesktopSshEnvironmentTarget } from "@circe/contracts";
import {
  describeReadinessCause,
  waitForHttpReady as waitForHttpReadyShared,
} from "@circe/shared/httpReadiness";
import { cliReleaseDownloadBaseUrl } from "@circe/shared/cliRelease";
import * as NetService from "@circe/shared/Net";
import { extractJsonObject, fromLenientJson } from "@circe/shared/schemaJson";
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env CIRCE_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@CIRCE_READY_TIMEOUT_MS@@"; then
    printf 'Remote Circe server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
    if [ -s "$LOG_FILE" ]; then
...
if [ "$REMOTE_MANAGED" != "external" ] && [ -n "$REMOTE_PID" ] && kill -0 "$REMOTE_PID" 2>/dev/null; then
  ...
  if kill -0 "$REMOTE_PID" 2>/dev/null; then
    printf 'Remote Circe server with PID %s did not stop within 2 seconds. Its ownership files were kept.\\n' "$REMOTE_PID" >&2
    exit 1
  fi
fi
rm -f "$PID_FILE" "$PORT_FILE" "$MANAGED_FILE"
...
>()("@circe/ssh/tunnel/SshEnvironmentManager") {
