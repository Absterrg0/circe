...
const makeFakeArchives = Effect.fn("test.makeFakeArchives")(function* () {
  ...
  const root = yield* fs.makeTempDirectoryScoped({ prefix: "circe-npm-packages-test-" });
  const archivesDir = path.join(root, "archives");
  yield* fs.makeDirectory(archivesDir);
  for (const key of KEYS) {
    const stem = `circe-${VERSION}-${key}`;
    ...
    yield* fs.writeFileString(
      path.join(contentDir, "circe"),
      `#!/bin/sh\necho "stub ${key} $*"\nexit 7\n`,
    );
    yield* fs.chmod(path.join(contentDir, "circe"), 0o755);
  ...
  it.effect("builds platform packages and a launcher that execs the installed one", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        ["@absterrg0/circe-darwin-arm64", "@absterrg0/circe-linux-x64", "@absterrg0/circe"],
      );
      for (const output of outputs) {
        assert.isTrue(yield* fs.exists(output.tarball), output.tarball);
      }

      const linuxDir = path.join(fixture.outputDir, "@absterrg0/circe-linux-x64");
      const linuxManifest = yield* decodeManifest(
        yield* fs.readFileString(path.join(linuxDir, "package.json")),
      );
      assert.equal(linuxManifest.name, "@absterrg0/circe-linux-x64");
      ...
      assert.deepStrictEqual(outputEntries.sort(), ["@absterrg0"]);
      ...
      const listing = yield* run(
        "tar",
        ["-tzvf", path.join(fixture.outputDir, "@absterrg0/circe-linux-x64.tgz")],
      ...
      assert.isTrue(
        lines.some((line) => /^-rwxr-xr-x .* package\/circe$/.test(line)),
        listing.stdout,
      );
      ...
      if (KEYS.some((key) => key === `${hostPlatform}-${hostArch}`)) {
        const passthrough = yield* run(process.execPath, ["bin/circe.js", "serve", "--port", "1234"], {
          cwd: launcherDir,
          env,
        });
        assert.equal(
        ...
        const unpack = yield* run(
          "tar",
          ["-xf", path.join(fixture.outputDir, "@absterrg0/circe.tgz"), "-C", installedLauncher],
          {
            cwd: fixture.root,
          },
        );
      ...
      const unsupported = yield* run(process.execPath, ["bin/circe.js", "--version"], {
        cwd: launcherDir,
      ...
      assert.include(unsupported.stderr, "https://github.com/Absterrg0/circe/releases");
    }),
  );
});
