 ...
 * behind `npx @absterrg0/circe` / `npm i -g @absterrg0/circe`: one
 * `@absterrg0/circe-<platformKey>` package per archive holding the archive's
 * contents verbatim, plus the `@absterrg0/circe` launcher that lists them as
 * optionalDependencies and execs the one npm installed. The bytes a user gets
 * from npm are therefore the release archive's, and running them needs neither
 * a Node runtime, npm, nor a native build.
 *
 * Output layout under `--output-dir`:
 *
 *   @absterrg0/circe-<platformKey>/      archive contents flattened + package.json
 *   @absterrg0/circe-<platformKey>.tgz   the same tree as an npm tarball
 *   @absterrg0/circe/                    launcher: package.json, bin/circe.js, README.md
 *   @absterrg0/circe.tgz                 the launcher as an npm tarball
 *
 ...
 */
import { legacyCliLauncherScript } from "@circe/shared/legacyCliLauncher";
...
import { fromJsonStringPretty } from "@circe/shared/schemaJson";
import { isCommandAvailable } from "@circe/shared/shell";
import serverPackageJson from "../apps/server/package.json" with { type: "json" };

import { windowsSystemTar } from "./build-cli-archive.ts";

export const NPM_PLATFORM_PACKAGE_SCOPE = "@absterrg0";
export const NPM_LAUNCHER_PACKAGE_NAME = "@absterrg0/circe";
...
export function npmPlatformPackageName(platformKey: CliArchivePlatformKey): string {
  return `${NPM_PLATFORM_PACKAGE_SCOPE}/circe-${platformKey}`;
}
...
export function npmPlatformPackageManifest(
  ...
  return {
    ...
    version,
    description: `Circe CLI executable for ${platformKey}`,
    license: serverPackageJson.license,
    repository: serverPackageJson.repository,
    os: [os],
    cpu: [cpu],
    files: ["circe", "circe.exe", "client", "resource-monitor", "node_modules"],
...
export function npmPlatformPackageReadme(platformKey: CliArchivePlatformKey): string {
  return [
    ...
    "",
    `The Circe CLI executable for ${platformKey}. Do not install this package directly:`,
    ...
    "```",
    "",
    "Source and documentation: https://github.com/Absterrg0/circe",
    "",
  ].join("\n");
}

/** package.json for the `@absterrg0/circe` launcher. No engines: bin/circe.js is trivial CJS. */
export function npmLauncherPackageManifest(
  ...
  return {
    ...
    version,
    description: "Circe CLI. Installs the self-contained executable for this platform.",
    license: serverPackageJson.license,
    repository: serverPackageJson.repository,
    bin: { circe: "./bin/circe.js" },
    files: ["bin", "dist"],
...
/**
 * The launcher every `npx @absterrg0/circe` runs. Plain CommonJS with no dependencies so it
...
let packageDir;
try {
  packageDir = dirname(require.resolve("${NPM_PLATFORM_PACKAGE_SCOPE}/circe-" + key + "/package.json"));
} catch {
  process.stderr.write(
    [
      "circe: no Circe CLI build is available for this platform (" + key + ").",
      "Supported platforms: " + SUPPORTED.join(", ") + ".",
      "If yours is listed, reinstall @absterrg0/circe so npm fetches its optional dependency.",
      "The desktop app and release archives are at https://github.com/Absterrg0/circe/releases",
      "",
    ].join("\\n"),
  );
  process.exit(1);
...
const executable = join(packageDir, process.platform === "win32" ? "circe.exe" : "circe");
const result = spawnSync(executable, process.argv.slice(2), { stdio: "inherit" });
if (result.error) {
  process.stderr.write("circe: failed to start " + executable + ": " + result.error.message + "\\n");
  process.exit(1);
}
...
const stagePlatformPackage = Effect.fn("stagePlatformPackage")(function* (input: {
  ...
  const executableName = input.key.startsWith("win32") ? "circe.exe" : "circe";
...
/** Writes the launcher package (package.json, bin/circe.js, README) and its tarball. */
const stageLauncherPackage = Effect.fn("stageLauncherPackage")(function* (input: {
  ...
  const launcherScript = path.join(stageDir, "bin/circe.js");
...
const command = Command.make(
  "build-npm-platform-packages",
  {
    archivesDir: Flag.string("archives-dir").pipe(
      Flag.withDescription("Directory holding the release's circe-<version>-<platform> archives."),
    ),
  ...
  Command.withDescription(
    "Build the @absterrg0/circe launcher and @absterrg0/circe-<platform> npm packages from CLI release archives.",
  ),
);
