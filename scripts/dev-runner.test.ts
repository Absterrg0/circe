...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit CIRCE_PORT_OFFSET when provided", () =>
      Effect.gen(function* () {
        const result = yield* resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "CIRCE_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("returns structured context for a negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(error.configKey, "CIRCE_PORT_OFFSET");
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("forwards the reusable auth token to web dev and removes it for desktop", () =>
      Effect.gen(function* () {
        const input = {
          baseEnv: { CIRCE_DEV_AUTH_TOKEN: "reusable-dev-auth-token-that-is-long-enough" },
          serverOffset: 0,
          webOffset: 0,
          t3Home: undefined,
          browser: undefined,
        ...
        assert.equal(web.CIRCE_DEV_AUTH_TOKEN, input.baseEnv.CIRCE_DEV_AUTH_TOKEN);
        assert.equal(desktop.CIRCE_DEV_AUTH_TOKEN, undefined);
      }),
    );
    it.effect("leaves the shared home implicit and disables browser auto-open", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CIRCE_HOME, undefined);
        assert.equal(env.CIRCE_NO_BROWSER, "1");
      }),
    );
    ...
    it.effect("allows browser auto-open to be explicitly enabled", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CIRCE_NO_BROWSER, "0");
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CIRCE_HOME, path.resolve("/tmp/custom-t3"));
        assert.equal(env.CIRCE_PORT, "4222");
        assert.equal(env.VITE_HTTP_URL, "http://localhost:4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.CIRCE_NO_BROWSER, "1");
        assert.equal(env.CIRCE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.CIRCE_LOG_WS_EVENTS, "1");
        assert.equal(env.CIRCE_HOST, "0.0.0.0");
    ...
    it.effect("supports an explicit web port independently from the server port", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CIRCE_PORT, "3002");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            CIRCE_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          t3Home: undefined,
        ...
        assert.equal(env.CIRCE_MODE, "web");
        assert.equal(env.CIRCE_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CIRCE_LOG_WS_EVENTS, "0");
      }),
    );
    ...
    it.effect("uses custom t3Home when provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CIRCE_HOME, path.resolve("/tmp/my-t3"));
      }),
    );
    ...
    it.effect("pins desktop dev to a stable backend port and websocket url", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CIRCE_MODE, undefined);
        assert.equal(env.CIRCE_NO_BROWSER, undefined);
        assert.equal(env.CIRCE_HOST, undefined);
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CIRCE_PORT, "13773");
        assert.equal(env.PORT, "5733");
      }),
    );
    ...
    for (const mode of ["dev", "dev:web"] as const) {
      it.effect(`leaves the client backend URLs unset in ${mode} mode`, () =>
        Effect.gen(function* () {
          ...
          assert.equal(env.CIRCE_SINGLE_ORIGIN_DEV, "1");
        }),
      );
    }
    ...
    it.effect("clears the single-origin marker in dev:desktop mode", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev:desktop",
          baseEnv: { CIRCE_SINGLE_ORIGIN_DEV: "1" },
          serverOffset: 0,
          webOffset: 0,
          t3Home: undefined,
          browser: undefined,
        ...
        assert.equal(env.CIRCE_SINGLE_ORIGIN_DEV, undefined);
    ...
    // --host configures the *backend* (CIRCE_HOST). It must not become Vite's
  ...
  describe("devPortProbeHosts", () => {
    ...
    // actually bind, so --host/CIRCE_HOST has to be probed as well.
    ...
    // Only the backend honours --host/CIRCE_HOST. Vite reads HOST (set for
  ...
  describe("runDevRunnerWithInput", () => {
    it.effect("preserves invalid configuration as the exact cause", () =>
      Effect.gen(function* () {
          ...
          Effect.provide(
            Layer.merge(
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({ env: { CIRCE_PORT_OFFSET: "not-an-integer" } }),
              ),
            ),
          ),
          Effect.flip,
        ...
        assert.deepStrictEqual(error.configKeys, ["CIRCE_PORT_OFFSET", "CIRCE_DEV_INSTANCE"]);
    ...
    // Base-dir precedence (--home-dir > worktree .circe > ambient CIRCE_HOME)
    ...
    it.effect("ignores an ambient CIRCE_HOME when no home is resolved", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { CIRCE_HOME: "/home/user/.circe" },
          serverOffset: 0,
          webOffset: 0,
          t3Home: undefined,
          browser: undefined,
    ...
    // defaults: an explicit CIRCE_BUNDLED_DEV (even "0") must pass through.
    describe("--share bundled dev default", () => {
          ...
          yield* runDevRunnerWithInput({
            ...
            Effect.provideService(
              ...
              input.ambientBundledDev === undefined
                ? {}
                : { CIRCE_BUNDLED_DEV: input.ambientBundledDev },
            ),
          );
      ...
      it.effect("defaults CIRCE_BUNDLED_DEV=1 for a shared run", () =>
        Effect.gen(function* () {
          const env = yield* shareSpawnedEnv({ ambientBundledDev: undefined });
          assert.equal(env?.CIRCE_BUNDLED_DEV, "1");
        }),
      );

      it.effect("keeps an explicit CIRCE_BUNDLED_DEV=0 opt-out", () =>
        Effect.gen(function* () {
          const env = yield* shareSpawnedEnv({ ambientBundledDev: "0" });
          assert.equal(env?.CIRCE_BUNDLED_DEV, "0");
        }),
      );

      it.effect("leaves CIRCE_BUNDLED_DEV unset without --share", () =>
        Effect.gen(function* () {
          let captured: Record<string, string | undefined> | undefined;
          const spawnerLayer = Layer.succeed(
            ChildProcessSpawner.ChildProcessSpawner,
          ...
          assert.equal(captured?.CIRCE_BUNDLED_DEV, undefined);
        }),
      );
    });
    ...
    describe("t3 home precedence", () => {
      ...
      const spawnedHome = (input: {
        ...
        Effect.gen(function* () {
          ...
          yield* runDevRunnerWithInput({ ...devServerInput, t3Home: input.t3Home }).pipe(
            ...
            Effect.provideService(
              HostProcessEnvironment,
              input.ambientHome === undefined ? {} : { CIRCE_HOME: input.ambientHome },
            ),
          );

          return captured?.CIRCE_HOME;
        });
      ...
      it.effect("treats a blank --home-dir as unset rather than as a selection", () =>
        Effect.gen(function* () {
          ...
          assert.equal(home, path.join(path.resolve(root), ".circe"));
        }).pipe(Effect.scoped),
      );

      it.effect("prefers the worktree .circe over an ambient CIRCE_HOME", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorktree;
          const home = yield* spawnedHome({
      ...
      it.effect("falls back to an ambient CIRCE_HOME outside a worktree", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const home = yield* spawnedHome({
            t3Home: undefined,
          ...
          assert.equal(home, path.resolve("/home/user/.circe"));
        }),
      );
