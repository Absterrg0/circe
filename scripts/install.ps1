...
#
#   irm https://heycirce.com/install.ps1 | iex
#
# Environment:
...
#   CIRCE_INSTALL_BIN_DIR   where circe.exe is linked (default: ~\.local\bin)
...
$circeHome = if ($env:CIRCE_HOME) { $env:CIRCE_HOME } else { Join-Path $HOME ".circe" }
$binDir = if ($env:CIRCE_INSTALL_BIN_DIR) { $env:CIRCE_INSTALL_BIN_DIR } else { Join-Path $HOME ".local\bin" }

function Fail([string] $message) {
  Write-Error "circe install: $message"
  exit 1
}
...
if (-not $version) {
  ...
  $releases = Invoke-RestMethod -Uri "https://api.github.com/repos/$repo/releases?per_page=100" -Headers @{ "User-Agent" = "circe-install" }
...
if ($version -match '-preview\.') {
  Write-Warning "circe $version is a preview build. Preview builds are cut by maintainers from unreleased branches to exercise the release pipeline. They can be broken, receive no fixes, and are never offered as updates. Set CIRCE_CHANNEL=stable (the default) for a supported build."
  if ($channel -ne "preview" -and -not $env:CIRCE_VERSION) {
    Fail "refusing a preview build that was not explicitly requested"
  }
}

$stem = "circe-$version-win32-$arch"
$archive = "$stem.zip"
...
if ((Test-Path $marker) -and ((Get-Content $marker -Raw).Trim() -eq $version)) {
  ...
  try {
    ...
    try {
      ...
      if ($status -eq 404) {
        Fail "circe $version has no release archive for win32-$arch; releases before the self-contained CLI can only be installed with 'npm install -g @absterrg0/circe@$version'"
      }
      throw
    }
    ...
    & (Join-Path $staging "circe.exe") --version | Out-Null
...
$shim = Join-Path $binDir "circe.cmd"
# UTF-8 without a BOM: cmd.exe reads the shim as-is, and ASCII would corrupt
# non-ASCII characters in the user's home path.
[System.IO.File]::WriteAllText($shim, "@echo off`r`n`"$(Join-Path $targetDir 'circe.exe')`" %*", (New-Object System.Text.UTF8Encoding $false))
if ($interactive) { [Console]::Error.Write("`r$esc[2K") }
[Console]::Error.WriteLine("  ${green}Installed Circe $version$reset`n")
if (($env:PATH -split ";") -notcontains $binDir) {
  Write-Host "  Add $binDir to your PATH, then run ${bold}circe$reset.`n"
} else {
  Write-Host "  Run ${bold}circe$reset to get started.`n"
}
