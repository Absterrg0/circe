...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.CIRCE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.CIRCE_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.CIRCE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VITE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.CIRCE_RELAY_URL).toBeUndefined();
    expect(env.VITE_CIRCE_RELAY_URL).toBeUndefined();
    expect(env.CIRCE_MOBILE_OTLP_TRACES_URL).toBeUndefined();
    expect(env.CIRCE_MOBILE_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.CIRCE_MOBILE_OTLP_TRACES_TOKEN).toBeUndefined();
    ...
    expect(env.CIRCE_RELAY_CLIENT_OTLP_TRACES_URL).toBeUndefined();
    expect(env.CIRCE_RELAY_CLIENT_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.CIRCE_RELAY_CLIENT_OTLP_TRACES_TOKEN).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "CIRCE_CLERK_PUBLISHABLE_KEY=pk_root\nCIRCE_CLERK_JWT_TEMPLATE=template_root\nCIRCE_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nCIRCE_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "CIRCE_CLERK_PUBLISHABLE_KEY=pk_local\nCIRCE_CLERK_JWT_TEMPLATE=template_local\nCIRCE_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nCIRCE_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).CIRCE_RELAY_URL).toBe(
      "https://local.example.test",
    );
    expect(
      loadRepoEnv({
        baseEnv: {
          CIRCE_CLERK_PUBLISHABLE_KEY: "pk_ci",
          CIRCE_CLERK_JWT_TEMPLATE: "template_ci",
          CIRCE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          CIRCE_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      CIRCE_RELAY_URL: "https://ci.example.test",
      VITE_CIRCE_RELAY_URL: "https://ci.example.test",
    });
  });
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        ...
        CIRCE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_canonical",
        VITE_CIRCE_RELAY_URL: "https://legacy.example.test",
    ...
    expect(
      loadRepoEnv({
        baseEnv: {
          ...
          CIRCE_RELAY_CLIENT_OTLP_TRACES_DATASET: "relay-client-traces",
          CIRCE_RELAY_CLIENT_OTLP_TRACES_TOKEN: "relay-client-token",
        },
    ...
    expect(
      loadRepoEnv({
        baseEnv: {
          ...
          CIRCE_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
          CIRCE_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
        },
        repoRoot: makeTemporaryDirectory(),
      }),
    ).toEqual({
      CIRCE_RELAY_URL: "https://relay.example.test",
      VITE_CIRCE_RELAY_URL: "https://relay.example.test",
      CIRCE_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
      CIRCE_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
      CIRCE_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
      EXPO_PUBLIC_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
