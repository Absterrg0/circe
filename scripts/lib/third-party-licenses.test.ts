...
async function createFixture(): Promise<{
  ...
  const root = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "circe-licenses-"));
  ...
  it("keeps the GhosttyKit notice pinned to the vendored framework revision", async () => {
    const [config, revision] = await Promise.all([
      ...
      NodeFSP.readFile(
        NodePath.join(
          REPOSITORY_ROOT,
          "apps/mobile/modules/circe-terminal/Vendor/libghostty/VERSION",
        ),
        "utf8",
      ),
    ]);
    ...
    expect(config).toContain(
      "https://github.com/Yash-Singh1/ghostty/tree/circe/custom-io-ordered-feed",
    );
  });
