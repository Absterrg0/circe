...
it("selects an explicit CI Android ABI without changing the local default", () => {
  ...
  assert.throws(() => resolveShowcaseAndroidAbi("mips"), /Unsupported CIRCE_SHOWCASE_ANDROID_ABI/u);
});
...
it("expands both appearances into independent upload-ready directories", () => {
  ...
  assert.deepStrictEqual(
    ...
    [
      {
        appearance: "light",
        directory: NodePath.join("/captures", "apple", "iphone-test", "light", "circe"),
      },
      {
        appearance: "dark",
        directory: NodePath.join("/captures", "apple", "iphone-test", "dark", "circe"),
      },
    ],
  );
});
...
it("defaults every device to the app's own palette", () => {
  assert.equal(DEFAULT_SHOWCASE_THEME, "circe");
  assert.equal(
