...
export function renderWindowsNodeSupervisorMjs(): string {
  return [
    ...
    'const stopMarker = process.env.CIRCE_NODE_STOP || join(home, "runtime", "windows-stop.marker");',
    'const childArgs = [join(runtimeRoot, "dist", "bin.mjs"), "--mode", "web", "--no-browser", "--port", "3773", "--circe-node-preset", "headless"];',
    'const childEnvironment = { ...process.env, CIRCE_NODE_PRESET: "headless" };',
    "let child = null;",
...
export function renderWindowsNodeLauncherCmd(): string {
  return [
    ...
    `set "CIRCE_NODE_PRESET=headless"`,
    `set "CIRCE_NODE_STOP=%CIRCE_HOME%\\runtime\\windows-stop.marker"`,
    'cd /d "%~dp0"',
    ":run_supervisor",
    'if exist "%CIRCE_NODE_STOP%" exit /b 0',
