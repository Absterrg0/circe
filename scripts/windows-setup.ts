...
// User-visible installer copy is Circe. The task name, install dir,
// registry paths, Circe.exe targets, and uninstall identity stay Circe
// so upgrades find the existing install.
export const WINDOWS_SETUP_TASK_NAME = "Circe Headless Node";
export const WINDOWS_SETUP_DATA_ROOT = "%USERPROFILE%\\.circe";
...
export function windowsSetupArtifactName(version: string, arch: WindowsSetupArch): string {
  ...
  return `Circe-Setup-${version}-win-${arch}.exe`;
}
...
export function renderWindowsNodeSupervisorMjs(): string {
  return [
    ...
    'const home = process.env.CIRCE_HOME || join(process.env.USERPROFILE || "", ".circe");',
    'const stopMarker = process.env.T3CODE_NODE_STOP || join(home, "runtime", "windows-stop.marker");',
    'const childArgs = [join(runtimeRoot, "dist", "bin.mjs"), "--mode", "web", "--no-browser", "--port", "3773", "--circe-node-preset", "headless"];',
    'const childEnvironment = { ...process.env, T3CODE_NODE_PRESET: "headless" };',
    "let child = null;",
...
/** Stop only Circe processes backed by the preserved Circe.exe owned by this install. */
export function renderWindowsOwnedProcessStopPs1(): string {
  return [
    ...
    "$taskkill = Join-Path $env:SystemRoot 'System32\\taskkill.exe'",
    "function Get-OwnedCirceProcess {",
    "  $candidates = @(Get-CimInstance Win32_Process -Filter \"Name = 'Circe.exe'\")",
    ...
    "}",
    "try {",
    "  $owned = @(Get-OwnedCirceProcess)",
    "  foreach ($process in $owned) {",
    "    & $taskkill /PID $process.ProcessId /T /F | Out-Null",
    "  }",
    "  for ($attempt = 0; $attempt -lt 50; $attempt++) {",
    "    $remaining = @(Get-OwnedCirceProcess)",
    "    if ($remaining.Count -eq 0) { exit 0 }",
    "    Start-Sleep -Milliseconds 100",
    "  }",
    "  $ids = ($remaining | ForEach-Object { $_.ProcessId }) -join ', '",
    '  Write-Error "Owned Circe processes remain after stop: $ids"',
    "  exit 1",
    "} catch {",
    '  Write-Error "Could not safely stop owned Circe processes: $($_.Exception.Message)"',
    "  exit 1",
    "}",
    "",
  ].join("\r\n");
...
export function renderWindowsNodeLauncherCmd(): string {
  return [
    ...
    `set "CIRCE_HOME=%USERPROFILE%\\.circe"`,
    `set "T3CODE_NODE_PRESET=headless"`,
    `set "T3CODE_NODE_STOP=%CIRCE_HOME%\\runtime\\windows-stop.marker"`,
    'cd /d "%~dp0"',
    ":run_supervisor",
    'if exist "%T3CODE_NODE_STOP%" exit /b 0',
...
export function renderWindowsSetupNsi(input: {
  ...
  const runtimeStopPs1 = stage("runtime-win\\circe-node-stop.ps1");
  const ownedProcessStopPs1 = stage("circe-owned-process-stop.ps1");
  const ownedProcessStopCommand = `"$OwnedProcessPowerShellPath" -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "$PLUGINSDIR\\circe-owned-process-stop.ps1" -DesktopPath "$INSTDIR\\desktop\\Circe.exe"`;
  const stopHeadlessNodeFunction = [
    "Function StopHeadlessNode",
    "  ClearErrors",
    '  StrCpy $StopHelperAvailable "0"',
    '  StrCpy $StopFailed "0"',
    '  CreateDirectory "$PROFILE\\.circe\\runtime"',
    '  FileOpen $0 "$PROFILE\\.circe\\runtime\\windows-stop.marker" w',
    "  FileClose $0",
    "  Sleep 1500",
    '  IfFileExists "$PLUGINSDIR\\circe-node-stop.ps1" stop_headless_plugin_helper stop_headless_installed_helper',
    "stop_headless_plugin_helper:",
    '  StrCpy $StopHelperAvailable "1"',
    '  StrCpy $StopHelperPath "$PLUGINSDIR\\circe-node-stop.ps1"',
    "  Goto stop_headless_helper_attempt",
    "stop_headless_installed_helper:",
    '  IfFileExists "$INSTDIR\\runtime-win\\circe-node-stop.ps1" 0 stop_headless_helper_end',
    '  StrCpy $StopHelperAvailable "1"',
    '  StrCpy $StopHelperPath "$INSTDIR\\runtime-win\\circe-node-stop.ps1"',
    ...
    `  nsExec::ExecToLog ${nsiQuote('$SYSDIR\\schtasks.exe /End /TN "Circe Headless Node"')}`,
    "  Pop $R9",
    ...
    `  nsExec::ExecToLog ${nsiQuote('$SYSDIR\\schtasks.exe /Delete /TN "Circe Headless Node" /F')}`,
    "  Pop $R9",
    '  StrCpy $R8 "0"',
    "stop_headless_task_query:",
    `  nsExec::ExecToLog ${nsiQuote('$SYSDIR\\schtasks.exe /Query /TN "Circe Headless Node"')}`,
    "  Pop $R9",
  ...
  const stopOwnedCirceProcessesFunction = [
    "Function StopOwnedCirceProcesses",
    "  ClearErrors",
    ...
    '  IfFileExists "$PLUGINSDIR\\circe-owned-process-stop.ps1" stop_owned_helper_ready stop_owned_helper_missing',
    ...
    "stop_owned_failed:",
    '  FileOpen $0 "$TEMP\\circe-owned-process-stop-diagnostic.txt" w',
    ...
    '  FileOpen $0 "$TEMP\\circe-owned-process-stop-diagnostic.txt" w',
    '  FileWrite $0 "missing=$PLUGINSDIR\\circe-owned-process-stop.ps1$\\r$\\n"',
    "  FileClose $0",
    '  DetailPrint "Owned process helper missing: $PLUGINSDIR\\circe-owned-process-stop.ps1"',
    "  SetErrors",
    "  Goto stop_owned_done",
    "stop_owned_success:",
    '  Delete "$TEMP\\circe-owned-process-stop-diagnostic.txt"',
    "stop_owned_done:",
    "  Return",
    "FunctionEnd",
    "",
  ];
  const uninstallStopOwnedCirceProcessesFunction = stopOwnedCirceProcessesFunction.map((line) =>
    line
      .replace("Function StopOwnedCirceProcesses", "Function un.StopOwnedCirceProcesses")
  ...
  return [
    ...
    '!define MUI_WELCOMEPAGE_TITLE "Welcome to Circe Setup"',
    ...
    '!define MUI_FINISHPAGE_RUN_TEXT "Launch Circe"',
    '!include "MUI2.nsh"',
    '!include "LogicLib.nsh"',
    '!include "nsDialogs.nsh"',
    '!include "FileFunc.nsh"',
    `Name "Circe ${input.version}"`,
    `OutFile ${nsiQuote(input.outputPath)}`,
    `BrandingText "Circe ${input.version}"`,
    'InstallDir "$LOCALAPPDATA\\Programs\\Circe"',
    'InstallDirRegKey HKCU "Software\\Circe" "InstallLocation"',
    "RequestExecutionLevel user",
    `VIProductVersion "${numericFileVersion}"`,
    `VIAddVersionKey /LANG=1033 "ProductName" "Circe"`,
    `VIAddVersionKey /LANG=1033 "ProductVersion" "${input.version}"`,
    `VIAddVersionKey /LANG=1033 "FileDescription" "Circe Node setup"`,
    ...
    "  Goto mode_validate",
    "mode_from_existing:",
    '  ReadINIStr $ExistingMode "$PROFILE\\.circe\\config\\preset.ini" Circe Preset',
    ...
    "    Abort",
    "  ${EndIf}",
    '  ${NSD_CreateLabel} 0 0 100% 18u "Choose how this Windows device runs Circe"',
    "  Pop $0",
    ...
    `  File /oname=circe-owned-process-stop.ps1 ${nsiQuote(ownedProcessStopPs1)}`,
    "SectionEnd",
    "",
    ...stopOwnedCirceProcessesFunction,
    ...uninstallStopOwnedCirceProcessesFunction,
    "",
    "Function LaunchSelectedProduct",
    '  StrCmp $NodeMode "headless" launch_selected_done 0',
    '  Exec "$INSTDIR\\desktop\\Circe.exe"',
    ...
    '  SetOutPath "$PLUGINSDIR"',
    `  File /oname=$PLUGINSDIR\\circe-node-stop.ps1 ${nsiQuote(runtimeStopPs1)}`,
    "SectionEnd",
    "",
    ...
    "apps_prompt:",
    '  MessageBox MB_ICONEXCLAMATION|MB_OKCANCEL "Close Circe before continuing. The installer will request it to exit." IDOK apps_close IDCANCEL apps_abort',
    "apps_abort:",
    "  Abort",
    "apps_close:",
    "  Call StopOwnedCirceProcesses",
    ...
    '  MessageBox MB_ICONSTOP "Circe could not stop its existing processes safely. Close Circe and try again."',
    "  Abort",
    "reset_stop_failed:",
    ...
    '  MessageBox MB_ICONSTOP "Circe could not stop the existing headless runtime. Your installation was left in place."',
    "  Abort",
    "reset_stop_done:",
    "SectionEnd",
    "",
    ...
    '  RMDir /r "$INSTDIR\\.incoming"',
    '  RMDir /r "$INSTDIR\\.previous"',
    '  CreateDirectory "$PROFILE\\.circe\\config"',
    '  CreateDirectory "$PROFILE\\.circe\\runtime"',
    '  WriteINIStr "$PROFILE\\.circe\\config\\preset.ini" Circe Preset $NodeMode',
    '  FileOpen $0 "$PROFILE\\.circe\\config\\node-preset.json" w',
    ...
    '  FileWrite $0 "{$\\"product$\\":$\\"Circe$\\",$\\"preset$\\":$\\"$NodeMode$\\",$\\"nodeType$\\":$\\"$NodeMode$\\"}$\\r$\\n"',
    "  FileClose $0",
    '  WriteRegStr HKCU "Software\\Circe" "InstallLocation" "$INSTDIR"',
    '  WriteRegStr HKCU "Software\\Circe" "Preset" "$NodeMode"',
    `  WriteRegStr HKCU "${WINDOWS_SETUP_UNINSTALL_REGISTRY_KEY}" "DisplayName" "Circe"`,
    ...
    "registry_gui:",
    '  StrCpy $0 "$INSTDIR\\desktop\\Circe.exe"',
    "registry_icon_ready:",
    `  WriteRegStr HKCU "${WINDOWS_SETUP_UNINSTALL_REGISTRY_KEY}" "DisplayIcon" "$0"`,
    `  WriteRegStr HKCU "${WINDOWS_SETUP_UNINSTALL_REGISTRY_KEY}" "UninstallString" "$\\"$INSTDIR\\Uninstall Circe.exe$\\""`,
    `  WriteRegStr HKCU "${WINDOWS_SETUP_UNINSTALL_REGISTRY_KEY}" "QuietUninstallString" "$\\"$INSTDIR\\Uninstall Circe.exe$\\" /S"`,
    ...
    '  WriteUninstaller "$INSTDIR\\Uninstall Circe.exe"',
    '  StrCmp $NodeMode "headless" 0 state_gui',
    '  FileOpen $0 "$PROFILE\\.circe\\runtime\\windows-stop.marker" w',
    "  FileClose $0",
    '  Delete "$PROFILE\\.circe\\runtime\\windows-stop.marker"',
    `  nsExec::ExecToLog ${nsiQuote(renderWindowsTaskCreateCommand("$INSTDIR\\runtime-win\\circe-node-launcher.cmd"))}`,
    "  Pop $R9",
    '  StrCmp $R9 "0" headless_task_created headless_task_create_failed',
    "headless_task_create_failed:",
    '  DetailPrint "Circe headless task registration failed: exit=$R9"',
    "  Goto headless_task_failed",
    "headless_task_created:",
    `  nsExec::ExecToLog ${nsiQuote('$SYSDIR\\schtasks.exe /Run /TN "Circe Headless Node"')}`,
    "  Pop $R9",
    '  StrCmp $R9 "0" headless_task_started headless_task_start_failed',
    "headless_task_start_failed:",
    '  DetailPrint "Circe headless task start failed: exit=$R9"',
    "  Goto headless_task_failed",
    ...
    '  MessageBox MB_ICONSTOP "Circe installed the headless payload but could not start its scheduled task. Start \\"Circe Headless Node\\" in Task Scheduler and check the service log."',
    "  Abort",
    "headless_task_started:",
    "  Goto state_done",
    "state_gui:",
    '  Delete "$PROFILE\\.circe\\runtime\\windows-stop.marker"',
    '  CreateDirectory "$SMPROGRAMS\\Circe"',
    `  CreateShortCut "$DESKTOP\\Circe.lnk" "$INSTDIR\\desktop\\Circe.exe"`,
    `  CreateShortCut "$SMPROGRAMS\\Circe\\Circe.lnk" "$INSTDIR\\desktop\\Circe.exe"`,
    '  Delete "$DESKTOP\\Circe.lnk"',
    '  Delete "$SMPROGRAMS\\Circe\\Circe.lnk"',
    '  RMDir "$SMPROGRAMS\\Circe"',
    "  Goto state_done",
    ...
    '  MessageBox MB_ICONSTOP "Circe could not validate the staged payload. Your existing installation was left in place."',
    "  Abort",
    "FunctionEnd",
    "",
    ...
    '  IfFileExists "$INSTDIR\\.incoming\\runtime-win\\circe-payload-complete.txt" 0 staged_payload_invalid',
    ...
    '  IfFileExists "$INSTDIR\\.incoming\\runtime-win\\circe-node-supervisor.mjs" 0 staged_payload_invalid',
    '  IfFileExists "$INSTDIR\\.incoming\\runtime-win\\circe-node-stop.ps1" 0 staged_payload_invalid',
    '  IfFileExists "$INSTDIR\\.incoming\\runtime-win\\circe-node-launcher.cmd" staged_payload_valid staged_payload_invalid',
    "  Goto staged_payload_invalid",
    "staged_payload_controller_or_full:",
    '  IfFileExists "$INSTDIR\\.incoming\\desktop\\circe-payload-complete.txt" 0 staged_payload_invalid',
    '  IfFileExists "$INSTDIR\\.incoming\\desktop\\Circe.exe" 0 staged_payload_invalid',
    ...
    "  Quit",
    "restore_interactive:",
    '  MessageBox MB_ICONSTOP "Circe could not commit the staged payload. Your existing installation was restored."',
    "  Abort",
    "restore_failed:",
    '  RMDir /r "$INSTDIR\\.incoming"',
    ...
    '  MessageBox MB_ICONSTOP "Circe could not fully roll back the staged payload. Recovery files, if present, remain at $INSTDIR\\.previous."',
    "  Abort",
    "FunctionEnd",
    "",
    'Section "Uninstall"',
    ...
    '  SetOutPath "$PLUGINSDIR"',
    `  File /oname=circe-owned-process-stop.ps1 ${nsiQuote(ownedProcessStopPs1)}`,
    "  Call un.StopOwnedCirceProcesses",
    ...
    '  MessageBox MB_ICONSTOP "Circe could not stop the headless runtime. The installation was left in place."',
    "  Abort",
    ...
    '  MessageBox MB_ICONSTOP "Circe could not stop its processes safely. The installation was left in place."',
    "  Abort",
    ...
    '  Delete "$SMPROGRAMS\\Circe\\Circe.lnk"',
    '  RMDir "$SMPROGRAMS\\Circe"',
    '  DeleteRegKey HKCU "Software\\Circe"',
    `  DeleteRegKey HKCU "${WINDOWS_SETUP_UNINSTALL_REGISTRY_KEY}"`,
    '  RMDir /r "$INSTDIR"',
    "  ; Deliberately preserve $PROFILE\\.circe: projects, providers, reports, and credentials.",
    "SectionEnd",
    "",
  ].join("\r\n");
}
